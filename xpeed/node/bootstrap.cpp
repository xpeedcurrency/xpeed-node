...
size_t constexpr xpeed::frontier_req_client::size_frontier;

xpeed::socket::socket (std::shared_ptr<xpeed::node> node_a) :
...
void xpeed::socket::async_connect (xpeed::tcp_endpoint const & endpoint_a, std::function<void(boost::system::error_code const &)> callback_a)
{
	checkup ();
...
void xpeed::socket::async_read (std::shared_ptr<std::vector<uint8_t>> buffer_a, size_t size_a, std::function<void(boost::system::error_code const &, size_t)> callback_a)
{
	...
	if (socket_m.is_open ())
	{
		...
		boost::asio::async_read (socket_m, boost::asio::buffer (buffer_a->data (), size_a), [this_l, callback_a](boost::system::error_code const & ec, size_t size_a) {
			this_l->node->stats.add (xpeed::stat::type::traffic_bootstrap, xpeed::stat::dir::in, size_a);
			this_l->stop ();
...
void xpeed::socket::async_write (std::shared_ptr<std::vector<uint8_t>> buffer_a, std::function<void(boost::system::error_code const &, size_t)> callback_a)
{
	auto this_l (shared_from_this ());
	if (socket_m.is_open ())
	{
		...
		boost::asio::async_write (socket_m, boost::asio::buffer (buffer_a->data (), buffer_a->size ()), [this_l, callback_a, buffer_a](boost::system::error_code const & ec, size_t size_a) {
			this_l->node->stats.add (xpeed::stat::type::traffic_bootstrap, xpeed::stat::dir::out, size_a);
			this_l->stop ();
			callback_a (ec, size_a);
		});
	}
...
void xpeed::socket::start (std::chrono::steady_clock::time_point timeout_a)
{
	cutoff = timeout_a.time_since_epoch ().count ();
}

void xpeed::socket::stop ()
{
	cutoff = std::numeric_limits<uint64_t>::max ();
}

void xpeed::socket::close ()
{
	if (socket_m.is_open ())
	{
		try
...
void xpeed::socket::checkup ()
{
	std::weak_ptr<xpeed::socket> this_w (shared_from_this ());
...
xpeed::tcp_endpoint xpeed::socket::remote_endpoint ()
{
...
xpeed::bootstrap_client::bootstrap_client (std::shared_ptr<xpeed::node> node_a, std::shared_ptr<xpeed::bootstrap_attempt> attempt_a, xpeed::tcp_endpoint const & endpoint_a) :
node (node_a),
attempt (attempt_a),
socket (std::make_shared<xpeed::socket> (node_a)),
...
xpeed::bootstrap_client::~bootstrap_client ()
{
	--attempt->connections;
}

double xpeed::bootstrap_client::block_rate () const
{
...
void xpeed::bootstrap_client::run ()
{
	auto this_l (shared_from_this ());
...
void xpeed::frontier_req_client::run ()
{
	std::unique_ptr<xpeed::frontier_req> request (new xpeed::frontier_req);
	request->start.clear ();
	...
	{
		xpeed::vectorstream stream (*send_buffer);
...
std::shared_ptr<xpeed::bootstrap_client> xpeed::bootstrap_client::shared ()
{
	return shared_from_this ();
}

xpeed::frontier_req_client::frontier_req_client (std::shared_ptr<xpeed::bootstrap_client> connection_a) :
...
xpeed::frontier_req_client::~frontier_req_client ()
{
}

void xpeed::frontier_req_client::receive_frontier ()
{
	auto this_l (shared_from_this ());
	connection->socket->async_read (connection->receive_buffer, xpeed::frontier_req_client::size_frontier, [this_l](boost::system::error_code const & ec, size_t size_a) {
		// An issue with asio is that sometimes, instead of reporting a bad file descriptor during disconnect,
		// we simply get a size of 0.
		if (size_a == xpeed::frontier_req_client::size_frontier)
		{
		...
		else
		{
			...
			{
				BOOST_LOG (this_l->connection->node->log) << boost::str (boost::format ("Invalid size: expected %1%, got %2%") % xpeed::frontier_req_client::size_frontier % size_a);
			}
		}
	});
}

void xpeed::frontier_req_client::unsynced (xpeed::block_hash const & head, xpeed::block_hash const & end)
{
...
void xpeed::frontier_req_client::received_frontier (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == xpeed::frontier_req_client::size_frontier);
		xpeed::account account;
		xpeed::bufferstream account_stream (connection->receive_buffer->data (), sizeof (account));
		auto error1 (xpeed::try_read (account_stream, account));
		assert (!error1);
		xpeed::block_hash latest;
		xpeed::bufferstream latest_stream (connection->receive_buffer->data () + sizeof (account), sizeof (latest));
		auto error2 (xpeed::try_read (latest_stream, latest));
		assert (!error2);
		if (count == 0)
		{
			start_time = std::chrono::steady_clock::now ();
			...
			if (!current.is_zero ())
			{
				if (account == current)
				{
					...
					else
					{
						...
						{
							connection->attempt->add_pull (xpeed::pull_info (account, latest, frontier));
				...
				else
				{
					...
					connection->attempt->add_pull (xpeed::pull_info (account, latest, xpeed::block_hash (0)));
				}
			}
			else
			{
...
void xpeed::frontier_req_client::next (xpeed::transaction const & transaction_a)
{
	...
	if (accounts.empty ())
	{
		...
		for (auto i (connection->node->store.latest_begin (transaction_a, current.number () + 1)), n (connection->node->store.latest_end ()); i != n && accounts.size () != max_size; ++i)
		{
			...
			accounts.push_back (std::make_pair (xpeed::account (i->first), info.head));
		}
		...
		{
			accounts.push_back (std::make_pair (xpeed::account (0), xpeed::block_hash (0)));
		}
	}
...
xpeed::bulk_pull_client::bulk_pull_client (std::shared_ptr<xpeed::bootstrap_client> connection_a, xpeed::pull_info const & pull_a) :
connection (connection_a),
known_account (0),
pull (pull_a),
total_blocks (0),
...
xpeed::bulk_pull_client::~bulk_pull_client ()
{
	// If received end block is not expected end block
	if (expected != pull.end)
	{
		pull.head = expected;
		if (connection->attempt->mode != xpeed::bootstrap_mode::legacy)
		{
...
void xpeed::bulk_pull_client::request ()
{
	expected = pull.head;
	xpeed::bulk_pull req;
...
void xpeed::bulk_pull_client::receive_block ()
{
	auto this_l (shared_from_this ());
...
void xpeed::bulk_pull_client::received_type ()
{
	auto this_l (shared_from_this ());
	xpeed::block_type type (static_cast<xpeed::block_type> (connection->receive_buffer->data ()[0]));
	switch (type)
	{
		case xpeed::block_type::send:
		{
			connection->socket->async_read (connection->receive_buffer, xpeed::send_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
		...
		{
			connection->socket->async_read (connection->receive_buffer, xpeed::receive_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			connection->socket->async_read (connection->receive_buffer, xpeed::open_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			connection->socket->async_read (connection->receive_buffer, xpeed::change_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			connection->socket->async_read (connection->receive_buffer, xpeed::state_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
...
void xpeed::bulk_pull_client::received_block (boost::system::error_code const & ec, size_t size_a, xpeed::block_type type_a)
{
	if (!ec)
	{
		xpeed::bufferstream stream (connection->receive_buffer->data (), size_a);
		std::shared_ptr<xpeed::block> block (xpeed::deserialize_block (stream, type_a));
		if (block != nullptr && !xpeed::work_validate (*block))
		{
			...
			if (!stop_pull && !connection->hard_stop.load ())
			{
				...
				to prevent spam */
				if (connection->attempt->mode != xpeed::bootstrap_mode::legacy || unexpected_count < 16384)
				{
					receive_block ();
				}
			}
...
xpeed::bulk_push_client::bulk_push_client (std::shared_ptr<xpeed::bootstrap_client> const & connection_a) :
connection (connection_a)
{
}

xpeed::bulk_push_client::~bulk_push_client ()
{
}

void xpeed::bulk_push_client::start ()
{
	xpeed::bulk_push message;
	auto buffer (std::make_shared<std::vector<uint8_t>> ());
	{
		xpeed::vectorstream stream (*buffer);
...
void xpeed::bulk_push_client::push (xpeed::transaction const & transaction_a)
{
	...
	while (block == nullptr && !finished)
	{
		...
		if (!finished)
		{
			...
			if (block == nullptr)
			{
				current_target.first = xpeed::block_hash (0);
			}
			else
			{
				if (connection->node->config.logging.bulk_pull_logging ())
...
void xpeed::bulk_push_client::send_finished ()
{
	auto buffer (std::make_shared<std::vector<uint8_t>> ());
	buffer->push_back (static_cast<uint8_t> (xpeed::block_type::not_a_block));
	connection->node->stats.inc (xpeed::stat::type::bootstrap, xpeed::stat::detail::bulk_push, xpeed::stat::dir::out);
	if (connection->node->config.logging.network_logging ())
	{
...
void xpeed::bulk_push_client::push_block (xpeed::block const & block_a)
{
	auto buffer (std::make_shared<std::vector<uint8_t>> ());
	{
		xpeed::vectorstream stream (*buffer);
		xpeed::serialize_block (stream, block_a);
	}
	auto this_l (shared_from_this ());
	connection->socket->async_write (buffer, [this_l](boost::system::error_code const & ec, size_t size_a) {
		if (!ec)
...
xpeed::bulk_pull_account_client::bulk_pull_account_client (std::shared_ptr<xpeed::bootstrap_client> connection_a, xpeed::account const & account_a) :
connection (connection_a),
account (account_a),
total_blocks (0)
{
...
xpeed::bulk_pull_account_client::~bulk_pull_account_client ()
{
	{
		std::lock_guard<std::mutex> mutex (connection->attempt->mutex);
		--connection->attempt->pulling;
...
void xpeed::bulk_pull_account_client::request ()
{
	xpeed::bulk_pull_account req;
	req.account = account;
	req.minimum_amount = connection->node->config.receive_minimum;
	req.flags = xpeed::bulk_pull_account_flags::pending_hash_and_amount;
...
void xpeed::bulk_pull_account_client::receive_pending ()
{
	auto this_l (shared_from_this ());
	size_t size_l (sizeof (xpeed::uint256_union) + sizeof (xpeed::uint128_union));
	connection->socket->async_read (connection->receive_buffer, size_l, [this_l, size_l](boost::system::error_code const & ec, size_t size_a) {
		...
		if (size_a == size_l)
		{
			if (!ec)
			{
				xpeed::block_hash pending;
				xpeed::bufferstream frontier_stream (this_l->connection->receive_buffer->data (), sizeof (xpeed::uint256_union));
				auto error1 (xpeed::try_read (frontier_stream, pending));
				assert (!error1);
				xpeed::amount balance;
				xpeed::bufferstream balance_stream (this_l->connection->receive_buffer->data () + sizeof (xpeed::uint256_union), sizeof (xpeed::uint128_union));
				auto error2 (xpeed::try_read (balance_stream, balance));
				assert (!error2);
...
xpeed::pull_info::pull_info (xpeed::account const & account_a, xpeed::block_hash const & head_a, xpeed::block_hash const & end_a, count_t count_a) :
account (account_a),
head (head_a),
end (end_a),
count (count_a),
...
xpeed::bootstrap_attempt::bootstrap_attempt (std::shared_ptr<xpeed::node> node_a) :
...
runs_count (0),
stopped (false),
mode (xpeed::bootstrap_mode::legacy),
lazy_stopped (0)
{
...
xpeed::bootstrap_attempt::~bootstrap_attempt ()
{
	BOOST_LOG (node->log) << "Exiting bootstrap attempt";
	node->bootstrap_initiator.notify_listeners (false);
}

bool xpeed::bootstrap_attempt::should_log ()
{
...
bool xpeed::bootstrap_attempt::request_frontier (std::unique_lock<std::mutex> & lock_a)
{
	auto result (true);
	...
	if (connection_l)
	{
		...
		{
			auto client (std::make_shared<xpeed::frontier_req_client> (connection_l));
			client->run ();
			frontiers = client;
...
void xpeed::bootstrap_attempt::request_pull (std::unique_lock<std::mutex> & lock_a)
{
	auto connection_l (connection (lock_a));
	if (connection_l)
	{
		...
		pulls.pop_front ();
		if (mode != xpeed::bootstrap_mode::legacy)
		{
		...
		node->background ([connection_l, pull]() {
			auto client (std::make_shared<xpeed::bulk_pull_client> (connection_l, pull));
			client->request ();
		});
	}
}

void xpeed::bootstrap_attempt::request_push (std::unique_lock<std::mutex> & lock_a)
{
	bool error (false);
	if (auto connection_shared = connection_frontier_request.lock ())
	{
		std::future<bool> future;
		{
			auto client (std::make_shared<xpeed::bulk_push_client> (connection_shared));
			client->start ();
			push = client;
			future = client->promise.get_future ();
		}
...
bool xpeed::bootstrap_attempt::still_pulling ()
{
...
void xpeed::bootstrap_attempt::run ()
{
	...
	if (!pulls.empty ())
	{
		...
		{
			auto k = xpeed::random_pool::generate_word32 (0, i);
	...
	if (!stopped)
	{
		...
		{
			lock.unlock ();
			mode = xpeed::bootstrap_mode::wallet_lazy;
			wallet_run ();
			lock.lock ();
		}
		...
		{
			lock.unlock ();
			mode = xpeed::bootstrap_mode::lazy;
			lazy_run ();
			lock.lock ();
		}
...
std::shared_ptr<xpeed::bootstrap_client> xpeed::bootstrap_attempt::connection (std::unique_lock<std::mutex> & lock_a)
{
	while (!stopped && idle.empty ())
	{
		condition.wait (lock_a);
	}
	std::shared_ptr<xpeed::bootstrap_client> result;
	if (!idle.empty ())
	{
		result = idle.back ();
		idle.pop_back ();
...
bool xpeed::bootstrap_attempt::consume_future (std::future<bool> & future_a)
{
	bool result;
	try
	{
...
{
	bool operator() (const std::shared_ptr<xpeed::bootstrap_client> & lhs, const std::shared_ptr<xpeed::bootstrap_client> & rhs) const
	{
		return lhs->block_rate () > rhs->block_rate ();
	}
};

unsigned xpeed::bootstrap_attempt::target_connections (size_t pulls_remaining)
{
...
void xpeed::bootstrap_attempt::populate_connections ()
{
	double rate_sum = 0.0;
	size_t num_pulls = 0;
	std::priority_queue<std::shared_ptr<xpeed::bootstrap_client>, std::vector<std::shared_ptr<xpeed::bootstrap_client>>, block_rate_cmp> sorted_connections;
	std::unordered_set<xpeed::tcp_endpoint> endpoints;
	{
		std::unique_lock<std::mutex> lock (mutex);
		num_pulls = pulls.size ();
		std::deque<std::weak_ptr<xpeed::bootstrap_client>> new_clients;
		for (auto & c : clients)
		{
	...
	if (connections < target)
	{
		...
		for (auto i = 0u; i < delta; i++)
		{
			...
			auto endpoint (xpeed::tcp_endpoint (peer.address (), peer.port ()));
			if (peer != xpeed::endpoint (boost::asio::ip::address_v6::any (), 0) && endpoints.find (endpoint) == endpoints.end ())
			{
				auto client (std::make_shared<xpeed::bootstrap_client> (node, shared_from_this (), endpoint));
				client->run ();
				std::lock_guard<std::mutex> lock (mutex);
	...
	if (!stopped)
	{
		std::weak_ptr<xpeed::bootstrap_attempt> this_w (shared_from_this ());
...
void xpeed::bootstrap_attempt::add_connection (xpeed::endpoint const & endpoint_a)
{
	auto client (std::make_shared<xpeed::bootstrap_client> (node, shared_from_this (), xpeed::tcp_endpoint (endpoint_a.address (), endpoint_a.port ())));
	client->run ();
}

void xpeed::bootstrap_attempt::pool_connection (std::shared_ptr<xpeed::bootstrap_client> client_a)
{
	{
		std::lock_guard<std::mutex> lock (mutex);
...
void xpeed::bootstrap_attempt::stop ()
{
	std::lock_guard<std::mutex> lock (mutex);
	stopped = true;
	condition.notify_all ();
...
void xpeed::bootstrap_attempt::add_pull (xpeed::pull_info const & pull)
{
	{
		std::lock_guard<std::mutex> lock (mutex);
...
void xpeed::bootstrap_attempt::requeue_pull (xpeed::pull_info const & pull_a)
{
	auto pull (pull_a);
	...
	else if (mode == xpeed::bootstrap_mode::lazy)
	{
		{
...
void xpeed::bootstrap_attempt::add_bulk_push_target (xpeed::block_hash const & head, xpeed::block_hash const & end)
{
	std::lock_guard<std::mutex> lock (mutex);
	bulk_push_targets.push_back (std::make_pair (head, end));
}

void xpeed::bootstrap_attempt::lazy_start (xpeed::block_hash const & hash_a)
{
	std::unique_lock<std::mutex> lock (lazy_mutex);
...
void xpeed::bootstrap_attempt::lazy_add (xpeed::block_hash const & hash_a)
{
...
void xpeed::bootstrap_attempt::lazy_pull_flush ()
{
	assert (!mutex.try_lock ());
	...
	for (auto & pull_start : lazy_pulls)
	{
		...
		{
			pulls.push_back (xpeed::pull_info (pull_start, pull_start, xpeed::block_hash (0), lazy_max_pull_blocks));
		}
	}
	lazy_pulls.clear ();
}

bool xpeed::bootstrap_attempt::lazy_finished ()
{
	bool result (true);
	auto transaction (node->store.tx_begin_read ());
	std::unique_lock<std::mutex> lock (lazy_mutex);
...
void xpeed::bootstrap_attempt::lazy_clear ()
{
	assert (!lazy_mutex.try_lock ());
	lazy_blocks.clear ();
	lazy_keys.clear ();
...
void xpeed::bootstrap_attempt::lazy_run ()
{
	populate_connections ();
...
bool xpeed::bootstrap_attempt::process_block (std::shared_ptr<xpeed::block> block_a, xpeed::account const & known_account_a, uint64_t total_blocks, bool block_expected)
{
	bool stop_pull (false);
	if (mode != xpeed::bootstrap_mode::legacy && block_expected)
	{
		...
		if (lazy_blocks.find (hash) == lazy_blocks.end ())
		{
			...
			{
				xpeed::uint128_t balance (std::numeric_limits<xpeed::uint128_t>::max ());
				xpeed::unchecked_info info (block_a, known_account_a, 0, xpeed::signature_verification::unknown);
				...
				else if (block_a->type () == xpeed::block_type::send)
				{
					// Calculate balance for legacy send blocks
					std::shared_ptr<xpeed::send_block> block_l (std::static_pointer_cast<xpeed::send_block> (block_a));
					if (block_l != nullptr)
					{
						balance = block_l->hashables.balance.number ();
					}
				}
				else if (block_a->type () == xpeed::block_type::state)
				{
					std::shared_ptr<xpeed::state_block> block_l (std::static_pointer_cast<xpeed::state_block> (block_a));
					if (block_l != nullptr)
					{
						balance = block_l->hashables.balance.number ();
						xpeed::block_hash link (block_l->hashables.link);
						...
						{
							xpeed::block_hash previous (block_l->hashables.previous);
							...
							{
								xpeed::amount prev_balance (node->ledger.balance (transaction, previous));
			...
			if (find_state != lazy_state_unknown.end ())
			{
				...
				if (block_a->type () == xpeed::block_type::state)
				{
					std::shared_ptr<xpeed::state_block> block_l (std::static_pointer_cast<xpeed::state_block> (block_a));
	...
	else if (mode != xpeed::bootstrap_mode::legacy)
	{
...
void xpeed::bootstrap_attempt::request_pending (std::unique_lock<std::mutex> & lock_a)
{
	auto connection_l (connection (lock_a));
	if (connection_l)
	{
		...
		node->background ([connection_l, account]() {
			auto client (std::make_shared<xpeed::bulk_pull_account_client> (connection_l, account));
			client->request ();
		});
	}
}

void xpeed::bootstrap_attempt::requeue_pending (xpeed::account const & account_a)
{
...
void xpeed::bootstrap_attempt::wallet_start (std::deque<xpeed::account> & accounts_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	wallet_accounts.swap (accounts_a);
}

bool xpeed::bootstrap_attempt::wallet_finished ()
{
	assert (!mutex.try_lock ());
	auto running (!stopped);
...
void xpeed::bootstrap_attempt::wallet_run ()
{
	populate_connections ();
	auto start_time (std::chrono::steady_clock::now ());
...
xpeed::bootstrap_initiator::bootstrap_initiator (xpeed::node & node_a) :
node (node_a),
stopped (false),
thread ([this]() {
	xpeed::thread_role::set (xpeed::thread_role::name::bootstrap_initiator);
	run_bootstrap ();
})
{
}
...
void xpeed::bootstrap_initiator::bootstrap ()
{
	std::unique_lock<std::mutex> lock (mutex);
	if (!stopped && attempt == nullptr)
	{
		node.stats.inc (xpeed::stat::type::bootstrap, xpeed::stat::detail::initiate, xpeed::stat::dir::out);
		attempt = std::make_shared<xpeed::bootstrap_attempt> (node.shared ());
		condition.notify_all ();
	}
}

void xpeed::bootstrap_initiator::bootstrap (xpeed::endpoint const & endpoint_a, bool add_to_peers)
{
	if (add_to_peers)
	{
		node.peers.insert (xpeed::map_endpoint_to_v6 (endpoint_a), xpeed::protocol_version);
	}
	std::unique_lock<std::mutex> lock (mutex);
	if (!stopped)
	{
...
void xpeed::bootstrap_initiator::bootstrap_lazy (xpeed::block_hash const & hash_a, bool force)
{
	{
		std::unique_lock<std::mutex> lock (mutex);
		if (force)
		...
		node.stats.inc (xpeed::stat::type::bootstrap, xpeed::stat::detail::initiate_lazy, xpeed::stat::dir::out);
		if (attempt == nullptr)
		{
			attempt = std::make_shared<xpeed::bootstrap_attempt> (node.shared ());
			attempt->mode = xpeed::bootstrap_mode::lazy;
		}
...
void xpeed::bootstrap_initiator::bootstrap_wallet (std::deque<xpeed::account> & accounts_a)
{
	{
		std::unique_lock<std::mutex> lock (mutex);
		node.stats.inc (xpeed::stat::type::bootstrap, xpeed::stat::detail::initiate_wallet_lazy, xpeed::stat::dir::out);
		if (attempt == nullptr)
		{
			attempt = std::make_shared<xpeed::bootstrap_attempt> (node.shared ());
			attempt->mode = xpeed::bootstrap_mode::wallet_lazy;
		}
...
void xpeed::bootstrap_initiator::run_bootstrap ()
{
	std::unique_lock<std::mutex> lock (mutex);
	while (!stopped)
	{
		...
		{
			lock.unlock ();
			if (attempt->mode == xpeed::bootstrap_mode::legacy)
			{
				attempt->run ();
			}
			else if (attempt->mode == xpeed::bootstrap_mode::lazy)
			{
				attempt->lazy_run ();
			}
			else
...
void xpeed::bootstrap_initiator::add_observer (std::function<void(bool)> const & observer_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	observers.push_back (observer_a);
}

bool xpeed::bootstrap_initiator::in_progress ()
{
	return current_attempt () != nullptr;
}

std::shared_ptr<xpeed::bootstrap_attempt> xpeed::bootstrap_initiator::current_attempt ()
{
	std::lock_guard<std::mutex> lock (mutex);
	return attempt;
}

void xpeed::bootstrap_initiator::stop ()
{
	{
		std::unique_lock<std::mutex> lock (mutex);
		stopped = true;
...
void xpeed::bootstrap_initiator::notify_listeners (bool in_progress_a)
{
...
xpeed::bootstrap_listener::bootstrap_listener (boost::asio::io_context & io_ctx_a, uint16_t port_a, xpeed::node & node_a) :
acceptor (io_ctx_a),
...
void xpeed::bootstrap_listener::start ()
{
...
void xpeed::bootstrap_listener::stop ()
{
...
void xpeed::bootstrap_listener::accept_connection ()
{
	if (acceptor.is_open ())
	{
		if (connections.size () < node.config.bootstrap_connections_max)
		{
			auto socket (std::make_shared<xpeed::socket> (node.shared ()));
...
void xpeed::bootstrap_listener::accept_action (boost::system::error_code const & ec, std::shared_ptr<xpeed::socket> socket_a)
{
	if (!ec)
	{
		auto connection (std::make_shared<xpeed::bootstrap_server> (socket_a, node.shared ()));
		{
			std::lock_guard<std::mutex> lock (mutex);
			if (acceptor.is_open ())
			{
...
boost::asio::ip::tcp::endpoint xpeed::bootstrap_listener::endpoint ()
{
...
xpeed::bootstrap_server::~bootstrap_server ()
{
	if (node->config.logging.bulk_pull_logging ())
	{
...
xpeed::bootstrap_server::bootstrap_server (std::shared_ptr<xpeed::socket> socket_a, std::shared_ptr<xpeed::node> node_a) :
receive_buffer (std::make_shared<std::vector<uint8_t>> ()),
socket (socket_a),
node (node_a)
{
...
void xpeed::bootstrap_server::receive ()
{
	auto this_l (shared_from_this ());
...
void xpeed::bootstrap_server::receive_header_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == 8);
		xpeed::bufferstream type_stream (receive_buffer->data (), size_a);
		auto error (false);
		xpeed::message_header header (error, type_stream);
		if (!error)
		{
			switch (header.type)
			{
				...
				{
					node->stats.inc (xpeed::stat::type::bootstrap, xpeed::stat::detail::bulk_pull, xpeed::stat::dir::in);
				...
				{
					node->stats.inc (xpeed::stat::type::bootstrap, xpeed::stat::detail::bulk_pull_account, xpeed::stat::dir::in);
					auto this_l (shared_from_this ());
					socket->async_read (receive_buffer, header.payload_length_bytes (), [this_l, header](boost::system::error_code const & ec, size_t size_a) {
				...
				{
					node->stats.inc (xpeed::stat::type::bootstrap, xpeed::stat::detail::frontier_req, xpeed::stat::dir::in);
					auto this_l (shared_from_this ());
					socket->async_read (receive_buffer, header.payload_length_bytes (), [this_l, header](boost::system::error_code const & ec, size_t size_a) {
				...
				{
					node->stats.inc (xpeed::stat::type::bootstrap, xpeed::stat::detail::bulk_push, xpeed::stat::dir::in);
					add_request (std::unique_ptr<xpeed::message> (new xpeed::bulk_push (header)));
					break;
				}
...
void xpeed::bootstrap_server::receive_bulk_pull_action (boost::system::error_code const & ec, size_t size_a, xpeed::message_header const & header_a)
{
	if (!ec)
	{
		auto error (false);
		xpeed::bufferstream stream (receive_buffer->data (), size_a);
		std::unique_ptr<xpeed::bulk_pull> request (new xpeed::bulk_pull (error, stream, header_a));
		if (!error)
		{
			if (node->config.logging.bulk_pull_logging ())
			{
			...
			add_request (std::unique_ptr<xpeed::message> (request.release ()));
			receive ();
		}
	}
}

void xpeed::bootstrap_server::receive_bulk_pull_account_action (boost::system::error_code const & ec, size_t size_a, xpeed::message_header const & header_a)
{
	if (!ec)
	{
		auto error (false);
		...
		xpeed::bufferstream stream (receive_buffer->data (), size_a);
		std::unique_ptr<xpeed::bulk_pull_account> request (new xpeed::bulk_pull_account (error, stream, header_a));
		if (!error)
		{
			if (node->config.logging.bulk_pull_logging ())
			{
				BOOST_LOG (node->log) << boost::str (boost::format ("Received bulk pull account for %1% with a minimum amount of %2%") % request->account.to_account () % xpeed::amount (request->minimum_amount).format_balance (xpeed::Mxpd_ratio, 10, true));
			}
			add_request (std::unique_ptr<xpeed::message> (request.release ()));
			receive ();
		}
...
void xpeed::bootstrap_server::receive_keepalive_action (boost::system::error_code const & ec, size_t size_a, xpeed::message_header const & header_a)
{
	if (!ec)
	{
		auto error (false);
		xpeed::bufferstream stream (receive_buffer->data (), header_a.payload_length_bytes ());
		std::unique_ptr<xpeed::keepalive> request (new xpeed::keepalive (error, stream, header_a));
		if (!error)
		{
			add_request (std::unique_ptr<xpeed::message> (request.release ()));
			receive ();
...
void xpeed::bootstrap_server::receive_frontier_req_action (boost::system::error_code const & ec, size_t size_a, xpeed::message_header const & header_a)
{
	if (!ec)
	{
		auto error (false);
		xpeed::bufferstream stream (receive_buffer->data (), header_a.payload_length_bytes ());
		std::unique_ptr<xpeed::frontier_req> request (new xpeed::frontier_req (error, stream, header_a));
		if (!error)
		{
			if (node->config.logging.bulk_pull_logging ())
			{
...
void xpeed::bootstrap_server::add_request (std::unique_ptr<xpeed::message> message_a)
{
	std::lock_guard<std::mutex> lock (mutex);
...
void xpeed::bootstrap_server::finish_request ()
{
	std::lock_guard<std::mutex> lock (mutex);
	requests.pop ();
	if (!requests.empty ())
...
namespace
{
class request_response_visitor : public xpeed::message_visitor
{
public:
	request_response_visitor (std::shared_ptr<xpeed::bootstrap_server> connection_a) :
	connection (connection_a)
	{
	}
	...
	void keepalive (xpeed::keepalive const & message_a) override
	{
		...
		connection->node->stats.inc (xpeed::stat::type::message, xpeed::stat::detail::keepalive, xpeed::stat::dir::in);
		...
		connection->socket->async_write (bytes, [connection = connection](boost::system::error_code const & ec, size_t size_a) {
			...
			{
				connection->node->stats.inc (xpeed::stat::type::message, xpeed::stat::detail::keepalive, xpeed::stat::dir::out);
				connection->finish_request ();
			}
		});
	}
	void publish (xpeed::publish const &) override
	{
		assert (false);
	}
	void confirm_req (xpeed::confirm_req const &) override
	{
		assert (false);
	}
	void confirm_ack (xpeed::confirm_ack const &) override
	{
		assert (false);
	}
	void bulk_pull (xpeed::bulk_pull const &) override
	{
		auto response (std::make_shared<xpeed::bulk_pull_server> (connection, std::unique_ptr<xpeed::bulk_pull> (static_cast<xpeed::bulk_pull *> (connection->requests.front ().release ()))));
		response->send_next ();
	}
	void bulk_pull_account (xpeed::bulk_pull_account const &) override
	{
		auto response (std::make_shared<xpeed::bulk_pull_account_server> (connection, std::unique_ptr<xpeed::bulk_pull_account> (static_cast<xpeed::bulk_pull_account *> (connection->requests.front ().release ()))));
		response->send_frontier ();
	}
	void bulk_push (xpeed::bulk_push const &) override
	{
		auto response (std::make_shared<xpeed::bulk_push_server> (connection));
		response->receive ();
	}
	void frontier_req (xpeed::frontier_req const &) override
	{
		auto response (std::make_shared<xpeed::frontier_req_server> (connection, std::unique_ptr<xpeed::frontier_req> (static_cast<xpeed::frontier_req *> (connection->requests.front ().release ()))));
		response->send_next ();
	}
	void node_id_handshake (xpeed::node_id_handshake const &) override
	{
		assert (false);
	}
...
void xpeed::bootstrap_server::run_next ()
{
 ...
 */
void xpeed::bulk_pull_server::set_current_end ()
{
	include_start = false;
...
void xpeed::bulk_pull_server::send_next ()
{
	auto block (get_next ());
	if (block != nullptr)
	{
		{
			send_buffer->clear ();
			xpeed::vectorstream stream (*send_buffer);
			xpeed::serialize_block (stream, *block);
		}
		auto this_l (shared_from_this ());
		if (connection->node->config.logging.bulk_pull_logging ())
		{
...
std::shared_ptr<xpeed::block> xpeed::bulk_pull_server::get_next ()
{
...
void xpeed::bulk_pull_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next ();
...
void xpeed::bulk_pull_server::send_finished ()
{
	send_buffer->clear ();
	send_buffer->push_back (static_cast<uint8_t> (xpeed::block_type::not_a_block));
	auto this_l (shared_from_this ());
	if (connection->node->config.logging.bulk_pull_logging ())
	{
...
void xpeed::bulk_pull_server::no_block_sent (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == 1);
...
xpeed::bulk_pull_server::bulk_pull_server (std::shared_ptr<xpeed::bootstrap_server> const & connection_a, std::unique_ptr<xpeed::bulk_pull> request_a) :
connection (connection_a),
 ...
 */
void xpeed::bulk_pull_account_server::set_params ()
{
	assert (request != nullptr);
	...
	if (request->flags == xpeed::bulk_pull_account_flags::pending_address_only)
	{
		pending_address_only = true;
	}
	else if (request->flags == xpeed::bulk_pull_account_flags::pending_hash_amount_and_address)
	{
		/**
	...
	else if (request->flags == xpeed::bulk_pull_account_flags::pending_hash_and_amount)
	{
...
void xpeed::bulk_pull_account_server::send_frontier ()
{
	/*
	...
	xpeed::uint128_union account_frontier_balance (account_frontier_balance_int);
	...
	send_buffer->clear ();
	{
		xpeed::vectorstream output_stream (*send_buffer);
...
void xpeed::bulk_pull_account_server::send_next_block ()
{
	/*
...
std::pair<std::unique_ptr<xpeed::pending_key>, std::unique_ptr<xpeed::pending_info>> xpeed::bulk_pull_account_server::get_next ()
{
	std::pair<std::unique_ptr<xpeed::pending_key>, std::unique_ptr<xpeed::pending_info>> result;
	...
	while (true)
	{
		...
		if (stream == xpeed::store_iterator<xpeed::pending_key, xpeed::pending_info> (nullptr))
		{
			break;
		}

		xpeed::pending_key key (stream->first);
		xpeed::pending_info info (stream->second);
		...
		result.first = std::unique_ptr<xpeed::pending_key> (new xpeed::pending_key (key));
		result.second = std::unique_ptr<xpeed::pending_info> (new xpeed::pending_info (info));
...
void xpeed::bulk_pull_account_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next_block ();
...
void xpeed::bulk_pull_account_server::send_finished ()
{
	/*
	...
	{
		xpeed::vectorstream output_stream (*send_buffer);
		xpeed::uint256_union account_zero (0);
		xpeed::uint128_union balance_zero (0);
...
void xpeed::bulk_pull_account_server::complete (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		if (pending_address_only)
...
xpeed::bulk_pull_account_server::bulk_pull_account_server (std::shared_ptr<xpeed::bootstrap_server> const & connection_a, std::unique_ptr<xpeed::bulk_pull_account> request_a) :
connection (connection_a),
request (std::move (request_a)),
send_buffer (std::make_shared<std::vector<uint8_t>> ()),
current_key (0, 0)
...
xpeed::bulk_push_server::bulk_push_server (std::shared_ptr<xpeed::bootstrap_server> const & connection_a) :
receive_buffer (std::make_shared<std::vector<uint8_t>> ()),
connection (connection_a)
{
	receive_buffer->resize (256);
...
void xpeed::bulk_push_server::receive ()
{
...
void xpeed::bulk_push_server::received_type ()
{
	auto this_l (shared_from_this ());
	xpeed::block_type type (static_cast<xpeed::block_type> (receive_buffer->data ()[0]));
	switch (type)
	{
		case xpeed::block_type::send:
		{
			connection->node->stats.inc (xpeed::stat::type::bootstrap, xpeed::stat::detail::send, xpeed::stat::dir::in);
			connection->socket->async_read (receive_buffer, xpeed::send_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		case xpeed::block_type::receive:
		{
			connection->node->stats.inc (xpeed::stat::type::bootstrap, xpeed::stat::detail::receive, xpeed::stat::dir::in);
			connection->socket->async_read (receive_buffer, xpeed::receive_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		case xpeed::block_type::open:
		{
			connection->node->stats.inc (xpeed::stat::type::bootstrap, xpeed::stat::detail::open, xpeed::stat::dir::in);
			connection->socket->async_read (receive_buffer, xpeed::open_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		case xpeed::block_type::change:
		{
			connection->node->stats.inc (xpeed::stat::type::bootstrap, xpeed::stat::detail::change, xpeed::stat::dir::in);
			connection->socket->async_read (receive_buffer, xpeed::change_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		case xpeed::block_type::state:
		{
			connection->node->stats.inc (xpeed::stat::type::bootstrap, xpeed::stat::detail::state_block, xpeed::stat::dir::in);
			connection->socket->async_read (receive_buffer, xpeed::state_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
...
void xpeed::bulk_push_server::received_block (boost::system::error_code const & ec, size_t size_a, xpeed::block_type type_a)
{
	if (!ec)
	{
		xpeed::bufferstream stream (receive_buffer->data (), size_a);
		auto block (xpeed::deserialize_block (stream, type_a));
		if (block != nullptr && !xpeed::work_validate (*block))
		{
...
xpeed::frontier_req_server::frontier_req_server (std::shared_ptr<xpeed::bootstrap_server> const & connection_a, std::unique_ptr<xpeed::frontier_req> request_a) :
connection (connection_a),
...
void xpeed::frontier_req_server::send_next ()
{
...
void xpeed::frontier_req_server::send_finished ()
{
	{
		send_buffer->clear ();
		xpeed::vectorstream stream (*send_buffer);
		xpeed::uint256_union zero (0);
...
void xpeed::frontier_req_server::no_block_sent (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		connection->finish_request ();
...
void xpeed::frontier_req_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		count++;
...
void xpeed::frontier_req_server::next ()
{
	// Filling accounts deque to prevent often read transactions
	if (accounts.empty ())
	{

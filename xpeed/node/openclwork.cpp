...
__kernel void xpd_work (__global ulong * attempt, __global ulong * result_a, __global uchar * item_a)
{
...
xpeed::opencl_environment::opencl_environment (bool & error_a)
{
	...
	{
		xpeed::opencl_platform platform;
		platform.platform = *i;
...
void xpeed::opencl_environment::dump (std::ostream & stream)
{
	auto index (0);
...
xpeed::opencl_config::opencl_config () :
platform (0),
device (0),
threads (1024 * 1024)
{
...
xpeed::opencl_config::opencl_config (unsigned platform_a, unsigned device_a, unsigned threads_a) :
platform (platform_a),
device (device_a),
threads (threads_a)
{
...
xpeed::error xpeed::opencl_config::serialize_json (xpeed::jsonconfig & json) const
{
...
xpeed::error xpeed::opencl_config::deserialize_json (xpeed::jsonconfig & json)
{
...
xpeed::opencl_work::opencl_work (bool & error_a, xpeed::opencl_config const & config_a, xpeed::opencl_environment & environment_a, xpeed::logging & logging_a) :
config (config_a),
context (0),
attempt_buffer (0),
result_buffer (0),
...
logging (logging_a)
{
	...
	if (!error_a)
	{
		...
		if (!error_a)
		{
			xpeed::random_pool::generate_block (reinterpret_cast<uint8_t *> (rand.s.data ()), rand.s.size () * sizeof (decltype (rand.s)::value_type));
			...
			if (!error_a)
			{
				...
				if (!error_a)
				{
					...
					if (!error_a)
					{
						...
						{
							cl_int item_error (0);
							size_t item_size (sizeof (xpeed::uint256_union));
							...
							if (!error_a)
							{
								...
								if (!error_a)
								{
									...
									if (!error_a)
									{
										...
										kernel = clCreateKernel (program, "xpd_work", &kernel_error);
...
boost::optional<uint64_t> xpeed::opencl_work::generate_work (xpeed::uint256_union const & root_a)
{
	...
	while (xpeed::work_validate (root_a, result) && !error)
	{
		result = rand.next ();
		...
		{
			cl_int write_error2 = clEnqueueWriteBuffer (queue, item_buffer, false, 0, sizeof (xpeed::uint256_union), root_a.bytes.data (), 0, nullptr, nullptr);
...
std::unique_ptr<xpeed::opencl_work> xpeed::opencl_work::create (bool create_a, xpeed::opencl_config const & config_a, xpeed::logging & logging_a)
{
	std::unique_ptr<xpeed::opencl_work> result;
	if (create_a)
	{
		auto error (false);
		...
		if (!error)
		{
			result.reset (new xpeed::opencl_work (error, config_a, environment, logging_a));
			if (error)
			{
				result.reset ();
			}

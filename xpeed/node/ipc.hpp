...
namespace ipc
{
	...
	{
	public:
		xpeed::error deserialize_json (xpeed::jsonconfig & json_a);
		xpeed::error serialize_json (xpeed::jsonconfig & json) const;
	...
	{
	public:
		ipc_server (xpeed::node & node, xpeed::rpc & rpc);
		virtual ~ipc_server ();
		void stop ();
	...
	private:
		...
		std::vector<std::shared_ptr<xpeed::ipc::transport>> transports;
	};
	...
	public:
		...
		xpeed::error connect (std::string const & path);

		/** Connect to a tcp socket synchronously */
		xpeed::error connect (std::string const & host, uint16_t port);

		/** Connect to a tcp socket asynchronously */
		void async_connect (std::string const & host, uint16_t port, std::function<void(xpeed::error)> callback);

		/** Write buffer asynchronously */
		void async_write (std::shared_ptr<std::vector<uint8_t>> buffer_a, std::function<void(xpeed::error, size_t)> callback_a);

		/** Read \p size_a bytes asynchronously */
		void async_read (std::shared_ptr<std::vector<uint8_t>> buffer_a, size_t size_a, std::function<void(xpeed::error, size_t)> callback_a);
		 ...
		 */
		std::shared_ptr<std::vector<uint8_t>> prepare_request (xpeed::ipc::payload_encoding encoding_a, std::string const & payload_a);

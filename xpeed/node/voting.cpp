...
xpeed::vote_generator::vote_generator (xpeed::node & node_a, std::chrono::milliseconds wait_a) :
node (node_a),
wait (wait_a),
stopped (false),
started (false),
...
void xpeed::vote_generator::add (xpeed::block_hash const & hash_a)
{
	{
...
void xpeed::vote_generator::stop ()
{
	std::unique_lock<std::mutex> lock (mutex);
	stopped = true;
...
void xpeed::vote_generator::send (std::unique_lock<std::mutex> & lock_a)
{
	std::vector<xpeed::block_hash> hashes_l;
	hashes_l.reserve (12);
	...
	lock_a.unlock ();
	{
		...
		node.wallets.foreach_representative (transaction, [this, &hashes_l, &transaction](xpeed::public_key const & pub_a, xpeed::raw_key const & prv_a) {
...
void xpeed::vote_generator::run ()
{
	xpeed::thread_role::set (xpeed::thread_role::name::voting);
	std::unique_lock<std::mutex> lock (mutex);
	started = true;
	lock.unlock ();
	condition.notify_all ();
...
void xpeed::votes_cache::add (std::shared_ptr<xpeed::vote> const & vote_a)
{
	std::lock_guard<std::mutex> lock (cache_mutex);
	for (auto & block : vote_a->blocks)
	{
		auto hash (boost::get<xpeed::block_hash> (block));
		...
		if (existing == cache.get<1> ().end ())
		{
			...
			auto inserted (cache.insert (xpeed::cached_votes{ std::chrono::steady_clock::now (), hash, std::vector<std::shared_ptr<xpeed::vote>> (1, vote_a) }));
			assert (inserted.second);
		}
		else
		{
			// Insert new votes (old hash)
			cache.get<1> ().modify (existing, [vote_a](xpeed::cached_votes & cache_a) {
...
std::vector<std::shared_ptr<xpeed::vote>> xpeed::votes_cache::find (xpeed::block_hash const & hash_a)
{
	std::vector<std::shared_ptr<xpeed::vote>> result;
	std::lock_guard<std::mutex> lock (cache_mutex);
...
void xpeed::votes_cache::remove (xpeed::block_hash const & hash_a)
{
	std::lock_guard<std::mutex> lock (cache_mutex);

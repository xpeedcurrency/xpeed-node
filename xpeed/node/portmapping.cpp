...
xpeed::port_mapping::port_mapping (xpeed::node & node_a) :
node (node_a),
devices (nullptr),
...
void xpeed::port_mapping::start ()
{
	check_mapping_loop ();
}

void xpeed::port_mapping::refresh_devices ()
{
...
void xpeed::port_mapping::refresh_mapping ()
{
	if (!xpeed::is_test_network)
	{
		std::lock_guard<std::mutex> lock (mutex);
...
int xpeed::port_mapping::check_mapping ()
{
	int result (3600);
	if (!xpeed::is_test_network)
	{
...
void xpeed::port_mapping::check_mapping_loop ()
{

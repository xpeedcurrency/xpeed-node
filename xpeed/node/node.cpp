...
double constexpr xpeed::node::price_max;
double constexpr xpeed::node::free_cutoff;
std::chrono::seconds constexpr xpeed::node::period;
std::chrono::seconds constexpr xpeed::node::cutoff;
std::chrono::seconds constexpr xpeed::node::syn_cookie_cutoff;
std::chrono::minutes constexpr xpeed::node::backup_interval;
std::chrono::seconds constexpr xpeed::node::search_pending_interval;
std::chrono::seconds constexpr xpeed::node::peer_interval;
std::chrono::hours constexpr xpeed::node::unchecked_cleanup_interval;
std::chrono::milliseconds constexpr xpeed::node::process_confirmed_interval;

int constexpr xpeed::port_mapping::mapping_timeout;
int constexpr xpeed::port_mapping::check_timeout;
unsigned constexpr xpeed::active_transactions::request_interval_ms;
size_t constexpr xpeed::active_transactions::max_broadcast_queue;
size_t constexpr xpeed::block_arrival::arrival_size_min;
std::chrono::seconds constexpr xpeed::block_arrival::arrival_time_min;
uint64_t constexpr xpeed::online_reps::weight_period;
uint64_t constexpr xpeed::online_reps::weight_samples;

namespace xpeed
{
extern unsigned char xpd_bootstrap_weights[];
extern size_t xpd_bootstrap_weights_size;
}

xpeed::network::network (xpeed::node & node_a, uint16_t port) :
buffer_container (node_a.stats, xpeed::network::buffer_size, 4096), // 2Mb receive buffer
socket (node_a.io_ctx, xpeed::endpoint (boost::asio::ip::address_v6::any (), port)),
resolver (node_a.io_ctx),
node (node_a),
on (true)
{
	boost::thread::attributes attrs;
	xpeed::thread_attributes::set (attrs);
	for (size_t i = 0; i < node.config.network_threads; ++i)
	{
		packet_processing_threads.push_back (boost::thread (attrs, [this]() {
			xpeed::thread_role::set (xpeed::thread_role::name::packet_processing);
			try
			{
				process_packets ();
			}
...
void xpeed::network::receive ()
{
	...
	socket.async_receive_from (boost::asio::buffer (data->buffer, xpeed::network::buffer_size), data->endpoint, [this, data](boost::system::error_code const & error, size_t size_a) {
		if (!error && this->on)
		{
			data->size = size_a;
...
void xpeed::network::process_packets ()
{
...
void xpeed::network::send_keepalive (xpeed::endpoint const & endpoint_a)
{
	assert (endpoint_a.address ().is_v6 ());
	xpeed::keepalive message;
	...
	send_buffer (bytes->data (), bytes->size (), endpoint_a, [bytes, node_w, endpoint_a](boost::system::error_code const & ec, size_t) {
		if (auto node_l = node_w.lock ())
		{
			...
			{
				node_l->stats.inc (xpeed::stat::type::message, xpeed::stat::detail::keepalive, xpeed::stat::dir::out);
			}
		}
	});
}

void xpeed::node::keepalive (std::string const & address_a, uint16_t port_a, bool preconfigured_peer_a)
{
	...
	network.resolver.async_resolve (boost::asio::ip::udp::resolver::query (address_a, std::to_string (port_a)), [node_l, address_a, port_a, preconfigured_peer_a](boost::system::error_code const & ec, boost::asio::ip::udp::resolver::iterator i_a) {
		if (!ec)
		{
			...
			{
				auto endpoint (xpeed::map_endpoint_to_v6 (i->endpoint ()));
				node_l->send_keepalive (endpoint);
				if (preconfigured_peer_a)
				{
					node_l->peers.insert (endpoint, xpeed::protocol_version, true);
				}
			}
		}
		else
...
void xpeed::network::send_node_id_handshake (xpeed::endpoint const & endpoint_a, boost::optional<xpeed::uint256_union> const & query, boost::optional<xpeed::uint256_union> const & respond_to)
{
	assert (endpoint_a.address ().is_v6 ());
	boost::optional<std::pair<xpeed::account, xpeed::signature>> response (boost::none);
	if (respond_to)
	{
		response = std::make_pair (node.node_id.pub, xpeed::sign_message (node.node_id.prv, node.node_id.pub, *respond_to));
		assert (!xpeed::validate_message (response->first, *respond_to, response->second));
	}
	xpeed::node_id_handshake message (query, response);
	auto bytes = message.to_bytes ();
	...
	node.stats.inc (xpeed::stat::type::message, xpeed::stat::detail::node_id_handshake, xpeed::stat::dir::out);
	std::weak_ptr<xpeed::node> node_w (node.shared ());
	send_buffer (bytes->data (), bytes->size (), endpoint_a, [bytes, node_w, endpoint_a](boost::system::error_code const & ec, size_t) {
		if (auto node_l = node_w.lock ())
		{
...
void xpeed::network::republish (xpeed::block_hash const & hash_a, std::shared_ptr<std::vector<uint8_t>> buffer_a, xpeed::endpoint endpoint_a)
{
	...
	send_buffer (buffer_a->data (), buffer_a->size (), endpoint_a, [node_w, endpoint_a](boost::system::error_code const & ec, size_t size) {
		if (auto node_l = node_w.lock ())
		{
			...
			{
				node_l->stats.inc (xpeed::stat::type::message, xpeed::stat::detail::publish, xpeed::stat::dir::out);
			}
		}
	});
}
...
template <typename T>
bool confirm_block (xpeed::transaction const & transaction_a, xpeed::node & node_a, T & list_a, std::shared_ptr<xpeed::block> block_a, bool also_publish)
{
	bool result (false);
	if (node_a.config.enable_voting)
	{
		...
		{
			// Generate new vote
			node_a.wallets.foreach_representative (transaction_a, [&result, &list_a, &node_a, &transaction_a, &hash](xpeed::public_key const & pub_a, xpeed::raw_key const & prv_a) {
				result = true;
				auto vote (node_a.store.vote_generate (transaction_a, pub_a, prv_a, std::vector<xpeed::block_hash> (1, hash)));
				xpeed::confirm_ack confirm (vote);
...
bool confirm_block (xpeed::transaction const & transaction_a, xpeed::node & node_a, xpeed::endpoint & peer_a, std::shared_ptr<xpeed::block> block_a, bool also_publish)
{
	std::array<xpeed::endpoint, 1> endpoints;
	endpoints[0] = peer_a;
	auto result (confirm_block (transaction_a, node_a, endpoints, std::move (block_a), also_publish));
	return result;
}

void xpeed::network::confirm_hashes (xpeed::transaction const & transaction_a, xpeed::endpoint const & peer_a, std::vector<xpeed::block_hash> blocks_bundle_a)
{
	if (node.config.enable_voting)
	{
		node.wallets.foreach_representative (transaction_a, [this, &blocks_bundle_a, &peer_a, &transaction_a](xpeed::public_key const & pub_a, xpeed::raw_key const & prv_a) {
...
bool xpeed::network::send_votes_cache (xpeed::block_hash const & hash_a, xpeed::endpoint const & peer_a)
{
	// Search in cache
...
void xpeed::network::republish_block (std::shared_ptr<xpeed::block> block)
{
	auto hash (block->hash ());
	auto list (node.peers.list_fanout ());
	xpeed::publish message (block);
	auto bytes = message.to_bytes ();
...
void xpeed::network::republish_block (std::shared_ptr<xpeed::block> block, xpeed::endpoint const & peer_a)
{
	auto hash (block->hash ());
	xpeed::publish message (block);
...
void xpeed::network::republish_block_batch (std::deque<std::shared_ptr<xpeed::block>> blocks_a, unsigned delay_a)
{
...
void xpeed::network::republish_vote (std::shared_ptr<xpeed::vote> vote_a)
{
	xpeed::confirm_ack confirm (vote_a);
...
void xpeed::network::broadcast_confirm_req (std::shared_ptr<xpeed::block> block_a)
{
	auto list (std::make_shared<std::vector<xpeed::peer_information>> (node.peers.representatives (std::numeric_limits<size_t>::max ())));
	if (list->empty () || node.peers.total_weight () < node.config.online_weight_minimum.number ())
	{
		...
		list = std::make_shared<std::vector<xpeed::peer_information>> (node.peers.list_vector (std::min (static_cast<size_t> (100), 2 * node.peers.size_sqrt ())));
	}
...
void xpeed::network::broadcast_confirm_req_base (std::shared_ptr<xpeed::block> block_a, std::shared_ptr<std::vector<xpeed::peer_information>> endpoints_a, unsigned delay_a, bool resumption)
{
...
void xpeed::network::broadcast_confirm_req_batch (std::unordered_map<xpeed::endpoint, std::vector<std::pair<xpeed::block_hash, xpeed::block_hash>>> request_bundle_a, unsigned delay_a, bool resumption)
{
	const size_t max_reps = 10;
	if (!resumption && node.config.logging.network_logging ())
	{
...
void xpeed::network::broadcast_confirm_req_batch (std::deque<std::pair<std::shared_ptr<xpeed::block>, std::shared_ptr<std::vector<xpeed::peer_information>>>> deque_a, unsigned delay_a)
{
...
void xpeed::network::send_confirm_req (xpeed::endpoint const & endpoint_a, std::shared_ptr<xpeed::block> block)
{
	xpeed::confirm_req message (block);
	auto bytes = message.to_bytes ();
	...
	std::weak_ptr<xpeed::node> node_w (node.shared ());
	node.stats.inc (xpeed::stat::type::message, xpeed::stat::detail::confirm_req, xpeed::stat::dir::out);
...
void xpeed::network::send_confirm_req_hashes (xpeed::endpoint const & endpoint_a, std::vector<std::pair<xpeed::block_hash, xpeed::block_hash>> const & roots_hashes_a)
{
	xpeed::confirm_req message (roots_hashes_a);
	std::vector<uint8_t> bytes;
	{
		xpeed::vectorstream stream (bytes);
		message.serialize (stream);
...
template <typename T>
void rep_query (xpeed::node & node_a, T const & peers_a)
{
	auto transaction (node_a.store.tx_begin_read ());
	std::shared_ptr<xpeed::block> block (node_a.store.block_random (transaction));
	auto hash (block->hash ());
	...
	std::weak_ptr<xpeed::node> node_w (node_a.shared ());
...
void rep_query (xpeed::node & node_a, xpeed::endpoint const & peers_a)
{
	std::array<xpeed::endpoint, 1> peers;
	peers[0] = peers_a;
...
namespace
{
class network_message_visitor : public xpeed::message_visitor
{
public:
	network_message_visitor (xpeed::node & node_a, xpeed::endpoint const & sender_a) :
	node (node_a),
	sender (sender_a)
	{
	}
	virtual ~network_message_visitor () = default;
	void keepalive (xpeed::keepalive const & message_a) override
	{
		if (node.config.logging.network_keepalive_logging ())
		{
			BOOST_LOG (node.log) << boost::str (boost::format ("Received keepalive message from %1%") % sender);
		}
		node.stats.inc (xpeed::stat::type::message, xpeed::stat::detail::keepalive, xpeed::stat::dir::in);
		if (node.peers.contacted (sender, message_a.header.version_using))
		{
			auto endpoint_l (xpeed::map_endpoint_to_v6 (sender));
	...
	void publish (xpeed::publish const & message_a) override
	{
		if (node.config.logging.network_message_logging ())
		{
			BOOST_LOG (node.log) << boost::str (boost::format ("Publish message from %1% for %2%") % sender % message_a.block->hash ().to_string ());
		}
		node.stats.inc (xpeed::stat::type::message, xpeed::stat::detail::publish, xpeed::stat::dir::in);
	...
	void confirm_req (xpeed::confirm_req const & message_a) override
	{
		if (node.config.logging.network_message_logging ())
		{
		...
		node.stats.inc (xpeed::stat::type::message, xpeed::stat::detail::confirm_req, xpeed::stat::dir::in);
		node.peers.contacted (sender, message_a.header.version_using);
		...
		if (node.config.enable_voting && node.wallets.reps_count)
		{
			if (message_a.block != nullptr)
			{
				...
				if (!node.network.send_votes_cache (hash, sender))
				{
					...
					auto successor (node.ledger.successor (transaction, xpeed::uint512_union (message_a.block->previous (), message_a.block->root ())));
			...
			{
				auto transaction (node.store.tx_begin_read ());
				std::vector<xpeed::block_hash> blocks_bundle;
	...
	void confirm_ack (xpeed::confirm_ack const & message_a) override
	{
		if (node.config.logging.network_message_logging ())
		{
		...
		node.stats.inc (xpeed::stat::type::message, xpeed::stat::detail::confirm_ack, xpeed::stat::dir::in);
		node.peers.contacted (sender, message_a.header.version_using);
	...
	void bulk_pull (xpeed::bulk_pull const &) override
	{
		assert (false);
	}
	void bulk_pull_account (xpeed::bulk_pull_account const &) override
	{
		assert (false);
	}
	void bulk_push (xpeed::bulk_push const &) override
	{
		assert (false);
	}
	void frontier_req (xpeed::frontier_req const &) override
	{
		assert (false);
	}
	void node_id_handshake (xpeed::node_id_handshake const & message_a) override
	{
		if (node.config.logging.network_node_id_handshake_logging ())
		{
		...
		auto endpoint_l (xpeed::map_endpoint_to_v6 (sender));
		boost::optional<xpeed::uint256_union> out_query;
		boost::optional<xpeed::uint256_union> out_respond_to;
		if (message_a.query)
		{
		...
		node.stats.inc (xpeed::stat::type::message, xpeed::stat::detail::node_id_handshake, xpeed::stat::dir::in);
	}
	xpeed::node & node;
	xpeed::endpoint sender;
};
}

void xpeed::network::receive_action (xpeed::udp_data * data_a, xpeed::endpoint const & local_endpoint_a)
{
	...
	else if (xpeed::reserved_address (data_a->endpoint, false) && !node.config.allow_local_peers)
	{
		allowed_sender = false;
	}
	if (allowed_sender)
	{
		...
		xpeed::message_parser parser (node.block_uniquer, node.vote_uniquer, visitor, node.work);
		parser.deserialize_buffer (data_a->buffer, data_a->size);
		if (parser.status != xpeed::message_parser::parse_status::success)
		{
			node.stats.inc (xpeed::stat::type::error);

			switch (parser.status)
			{
				case xpeed::message_parser::parse_status::insufficient_work:
					// We've already increment error count, update detail only
					node.stats.inc_detail_only (xpeed::stat::type::error, xpeed::stat::detail::insufficient_work);
					break;
				case xpeed::message_parser::parse_status::invalid_magic:
					node.stats.inc (xpeed::stat::type::udp, xpeed::stat::detail::invalid_magic);
					break;
				case xpeed::message_parser::parse_status::invalid_network:
					node.stats.inc (xpeed::stat::type::udp, xpeed::stat::detail::invalid_network);
					break;
				case xpeed::message_parser::parse_status::invalid_header:
					node.stats.inc (xpeed::stat::type::udp, xpeed::stat::detail::invalid_header);
					break;
				case xpeed::message_parser::parse_status::invalid_message_type:
					node.stats.inc (xpeed::stat::type::udp, xpeed::stat::detail::invalid_message_type);
					break;
				case xpeed::message_parser::parse_status::invalid_keepalive_message:
					node.stats.inc (xpeed::stat::type::udp, xpeed::stat::detail::invalid_keepalive_message);
					break;
				case xpeed::message_parser::parse_status::invalid_publish_message:
					node.stats.inc (xpeed::stat::type::udp, xpeed::stat::detail::invalid_publish_message);
					break;
				case xpeed::message_parser::parse_status::invalid_confirm_req_message:
					node.stats.inc (xpeed::stat::type::udp, xpeed::stat::detail::invalid_confirm_req_message);
					break;
				case xpeed::message_parser::parse_status::invalid_confirm_ack_message:
					node.stats.inc (xpeed::stat::type::udp, xpeed::stat::detail::invalid_confirm_ack_message);
					break;
				case xpeed::message_parser::parse_status::invalid_node_id_handshake_message:
					node.stats.inc (xpeed::stat::type::udp, xpeed::stat::detail::invalid_node_id_handshake_message);
					break;
				case xpeed::message_parser::parse_status::outdated_version:
					node.stats.inc (xpeed::stat::type::udp, xpeed::stat::detail::outdated_version);
					break;
			...
			if (node.config.logging.network_logging () && parser.status != xpeed::message_parser::parse_status::outdated_version)
			{
		...
		{
			node.stats.add (xpeed::stat::type::traffic, xpeed::stat::dir::in, data_a->size);
		}
	}
	else
	{
		...
		node.stats.inc_detail_only (xpeed::stat::type::error, xpeed::stat::detail::bad_sender);
	}
}

// Send keepalives to all the peers we've been notified of
void xpeed::network::merge_peers (std::array<xpeed::endpoint, 8> const & peers_a)
{
...
bool xpeed::operation::operator> (xpeed::operation const & other_a) const
{
	return wakeup > other_a.wakeup;
}

xpeed::alarm::alarm (boost::asio::io_context & io_ctx_a) :
io_ctx (io_ctx_a),
thread ([this]() {
	xpeed::thread_role::set (xpeed::thread_role::name::alarm);
	run ();
})
{
}
...
void xpeed::alarm::add (std::chrono::steady_clock::time_point const & wakeup_a, std::function<void()> const & operation)
{
	{
		std::lock_guard<std::mutex> lock (mutex);
		operations.push (xpeed::operation ({ wakeup_a, operation }));
	}
...
xpeed::node_init::node_init () :
block_store_init (false),
wallet_init (false)
{
}

bool xpeed::node_init::error ()
{
	return block_store_init || wallet_init || wallets_store_init;
}

xpeed::vote_processor::vote_processor (xpeed::node & node_a) :
node (node_a),
started (false),
stopped (false),
active (false),
thread ([this]() {
	xpeed::thread_role::set (xpeed::thread_role::name::vote_processing);
	process_loop ();
})
{
	std::unique_lock<std::mutex> lock (mutex);
...
void xpeed::vote_processor::process_loop ()
{
	...
	while (!stopped)
	{
		if (!votes.empty ())
		{
			std::deque<std::pair<std::shared_ptr<xpeed::vote>, xpeed::endpoint>> votes_l;
			votes_l.swap (votes);
...
void xpeed::vote_processor::vote (std::shared_ptr<xpeed::vote> vote_a, xpeed::endpoint endpoint_a)
{
	assert (endpoint_a.address ().is_v6 ());
	std::unique_lock<std::mutex> lock (mutex);
	if (!stopped)
	{
		...
		{
			node.stats.inc (xpeed::stat::type::vote, xpeed::stat::detail::vote_overflow);
...
void xpeed::vote_processor::verify_votes (std::deque<std::pair<std::shared_ptr<xpeed::vote>, xpeed::endpoint>> & votes_a)
{
	...
	hashes.reserve (size);
	std::vector<size_t> lengths (size, sizeof (xpeed::uint256_union));
...
xpeed::vote_code xpeed::vote_processor::vote_blocking (xpeed::transaction const & transaction_a, std::shared_ptr<xpeed::vote> vote_a, xpeed::endpoint endpoint_a, bool validated)
{
	assert (endpoint_a.address ().is_v6 ());
	assert (!node.active.mutex.try_lock ());
	auto result (xpeed::vote_code::invalid);
	if (validated || !vote_a->validate ())
	{
		auto max_vote (node.store.vote_max (transaction_a, vote_a));
		result = xpeed::vote_code::replay;
		if (!node.active.vote (vote_a, true))
		{
			result = xpeed::vote_code::vote;
		}
		switch (result)
		{
			case xpeed::vote_code::vote:
				node.observers.vote.notify (transaction_a, vote_a, endpoint_a);
			case xpeed::vote_code::replay:
				...
				{
					xpeed::confirm_ack confirm (max_vote);
					node.network.confirm_send (confirm, confirm.to_bytes (), endpoint_a);
				}
				break;
			case xpeed::vote_code::invalid:
				assert (false);
				break;
		}
	}
	...
	switch (result)
	{
		case xpeed::vote_code::invalid:
			status = "Invalid";
			node.stats.inc (xpeed::stat::type::vote, xpeed::stat::detail::vote_invalid);
			break;
		case xpeed::vote_code::replay:
			status = "Replay";
			node.stats.inc (xpeed::stat::type::vote, xpeed::stat::detail::vote_replay);
			break;
		case xpeed::vote_code::vote:
			status = "Vote";
			node.stats.inc (xpeed::stat::type::vote, xpeed::stat::detail::vote_valid);
			break;
	}
	if (node.config.logging.vote_logging ())
	{
...
void xpeed::vote_processor::stop ()
{
	{
		std::lock_guard<std::mutex> lock (mutex);
		stopped = true;
...
void xpeed::vote_processor::flush ()
{
	std::unique_lock<std::mutex> lock (mutex);
...
void xpeed::vote_processor::calculate_weights ()
{
	std::unique_lock<std::mutex> lock (mutex);
	if (!stopped)
	{
		...
		{
			xpeed::account representative (i->first);
...
void xpeed::rep_crawler::add (xpeed::block_hash const & hash_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	active.insert (hash_a);
}

void xpeed::rep_crawler::remove (xpeed::block_hash const & hash_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	active.erase (hash_a);
}

bool xpeed::rep_crawler::exists (xpeed::block_hash const & hash_a)
{
	std::lock_guard<std::mutex> lock (mutex);
...
xpeed::node::node (xpeed::node_init & init_a, boost::asio::io_context & io_ctx_a, uint16_t peering_port_a, boost::filesystem::path const & application_path_a, xpeed::alarm & alarm_a, xpeed::logging const & logging_a, xpeed::work_pool & work_a) :
node (init_a, io_ctx_a, application_path_a, alarm_a, xpeed::node_config (peering_port_a, logging_a), work_a)
{
}

xpeed::node::node (xpeed::node_init & init_a, boost::asio::io_context & io_ctx_a, boost::filesystem::path const & application_path_a, xpeed::alarm & alarm_a, xpeed::node_config const & config_a, xpeed::work_pool & work_a, xpeed::node_flags flags_a) :
io_ctx (io_ctx_a),
config (config_a),
flags (flags_a),
alarm (alarm_a),
work (work_a),
store_impl (std::make_unique<xpeed::mdb_store> (init_a.block_store_init, config.logging, application_path_a / "data.ldb", config_a.lmdb_max_dbs, !flags.disable_unchecked_drop, flags.sideband_batch_size)),
store (*store_impl),
wallets_store_impl (std::make_unique<xpeed::mdb_wallets_store> (init_a.wallets_store_init, application_path_a / "wallets.ldb", config_a.lmdb_max_dbs)),
...
block_processor_thread ([this]() {
	xpeed::thread_role::set (xpeed::thread_role::name::block_processing);
...
startup_time (std::chrono::steady_clock::now ())
{
	...
	peers.peer_observer = [this](xpeed::endpoint const & endpoint_a) {
	...
	{
		observers.blocks.add ([this](std::shared_ptr<xpeed::block> block_a, xpeed::account const & account_a, xpeed::amount const & amount_a, bool is_state_send_a) {
				...
				background ([node_l, block_a, account_a, amount_a, is_state_send_a]() {
					...
					resolver->async_resolve (boost::asio::ip::tcp::resolver::query (address, std::to_string (port)), [node_l, address, port, target, body, resolver](boost::system::error_code const & ec, boost::asio::ip::tcp::resolver::iterator i_a) {
						...
						else
						{
							...
							node_l->stats.inc (xpeed::stat::type::error, xpeed::stat::detail::http_callback, xpeed::stat::dir::out);
						}
					});
				});
			}
	...
	observers.endpoint.add ([this](xpeed::endpoint const & endpoint_a) {
		this->network.send_keepalive (endpoint_a);
		rep_query (*this, endpoint_a);
	});
	observers.vote.add ([this](xpeed::transaction const & transaction, std::shared_ptr<xpeed::vote> vote_a, xpeed::endpoint const & endpoint_a) {
		assert (endpoint_a.address ().is_v6 ());
		this->gap_cache.vote (vote_a);
		this->online_reps.observe (vote_a->account);
		xpeed::uint128_t rep_weight;
		xpeed::uint128_t min_rep_weight;
		{
	...
	{
		BOOST_LOG (log) << "Node starting, version: " << XPD_MAJOR_MINOR_VERSION;
	}
	else
	{
		BOOST_LOG (log) << "Node starting, version: " << XPD_MAJOR_MINOR_RC_VERSION;
	}
	...
	if (!init_a.error ())
	{
		...
		node_id = xpeed::keypair (store.get_node_id (transaction));
		BOOST_LOG (log) << "Node ID: " << node_id.pub.to_account ();
	}
	peers.online_weight_minimum = config.online_weight_minimum.number ();
	if (xpeed::is_live_network || xpeed::is_beta_network)
	{
		xpeed::bufferstream weight_stream ((const uint8_t *)xpd_bootstrap_weights, xpd_bootstrap_weights_size);
		xpeed::uint128_union block_height;
		if (!xpeed::try_read (weight_stream, block_height))
		{
			...
			if (ledger.store.block_count (transaction).sum () < max_blocks)
			{
				...
				while (true)
				{
					...
					if (xpeed::try_read (weight_stream, account.bytes))
					{
						break;
					}
					xpeed::amount weight;
					if (xpeed::try_read (weight_stream, weight.bytes))
					{
						break;
					}
					BOOST_LOG (log) << "Using bootstrap rep weight: " << account.to_account () << " -> " << weight.format_balance (Mxpd_ratio, 0, true) << " XPD";
...
void xpeed::node::do_rpc_callback (boost::asio::ip::tcp::resolver::iterator i_a, std::string const & address, uint16_t port, std::shared_ptr<std::string> target, std::shared_ptr<std::string> body, std::shared_ptr<boost::asio::ip::tcp::resolver> resolver)
{
					...
					if (!ec)
					{
						...
						boost::beast::http::async_read (*sock, *sb, *resp, [node_l, sb, resp, sock, address, port, i_a, target, body, resolver](boost::system::error_code const & ec, size_t bytes_transferred) mutable {
							if (!ec)
							{
								...
								{
									node_l->stats.inc (xpeed::stat::type::http_callback, xpeed::stat::detail::initiate, xpeed::stat::dir::out);
								}
								else
								{
									if (node_l->config.logging.callback_logging ())
...
bool xpeed::node::copy_with_compaction (boost::filesystem::path const & destination_file)
{
	return !mdb_env_copy2 (boost::polymorphic_downcast<xpeed::mdb_store *> (store_impl.get ())->env.environment, destination_file.string ().c_str (), MDB_CP_COMPACT);
}

void xpeed::node::send_keepalive (xpeed::endpoint const & endpoint_a)
{
	network.send_keepalive (xpeed::map_endpoint_to_v6 (endpoint_a));
}

void xpeed::node::process_fork (xpeed::transaction const & transaction_a, std::shared_ptr<xpeed::block> block_a)
{
	auto root (block_a->root ());
	if (!store.block_exists (transaction_a, block_a->type (), block_a->hash ()) && store.root_exists (transaction_a, block_a->root ()))
	{
		std::shared_ptr<xpeed::block> ledger_block (ledger.forked_block (transaction_a, *block_a));
		if (ledger_block)
		{
			std::weak_ptr<xpeed::node> this_w (shared_from_this ());
			if (!active.start (ledger_block, [this_w, root](std::shared_ptr<xpeed::block>) {
				    if (auto this_l = this_w.lock ())
				    {
					    auto attempt (this_l->bootstrap_initiator.current_attempt ());
					    if (attempt && attempt->mode == xpeed::bootstrap_mode::legacy)
					    {
						    auto transaction (this_l->store.tx_begin_read ());
						    auto account (this_l->ledger.store.frontier_get (transaction, root));
						    if (!account.is_zero ())
						    {
							    attempt->requeue_pull (xpeed::pull_info (account, root, root));
						    }
						    else if (this_l->ledger.store.account_exists (transaction, root))
						    {
							    attempt->requeue_pull (xpeed::pull_info (root, xpeed::block_hash (0), xpeed::block_hash (0)));
						    }
					    }
				    }
			    }))
...
xpeed::gap_cache::gap_cache (xpeed::node & node_a) :
node (node_a)
{
}

void xpeed::gap_cache::add (xpeed::transaction const & transaction_a, xpeed::block_hash const & hash_a, std::chrono::steady_clock::time_point time_point_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	auto existing (blocks.get<1> ().find (hash_a));
	if (existing != blocks.get<1> ().end ())
	{
		blocks.get<1> ().modify (existing, [time_point_a](xpeed::gap_information & info) {
			info.arrival = time_point_a;
		});
	}
	else
	{
		blocks.insert ({ time_point_a, hash_a, std::unordered_set<xpeed::account> () });
		if (blocks.size () > max)
		{
...
void xpeed::gap_cache::vote (std::shared_ptr<xpeed::vote> vote_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	auto transaction (node.store.tx_begin_read ());
	for (auto hash : *vote_a)
	{
		...
		{
			auto is_new (false);
			blocks.get<1> ().modify (existing, [&](xpeed::gap_information & info) { is_new = info.voters.insert (vote_a->account).second; });
			if (is_new)
			{
				uint128_t tally;
				...
				if (start_bootstrap)
				{
					...
					node.alarm.add (xpeed::is_test_network ? now + std::chrono::milliseconds (5) : now + std::chrono::seconds (5), [node_l, hash]() {
...
xpeed::uint128_t xpeed::gap_cache::bootstrap_threshold (xpeed::transaction const & transaction_a)
{
	auto result ((node.online_reps.online_stake () / 256) * node.config.bootstrap_fraction_numerator);
	return result;
}

size_t xpeed::gap_cache::size ()
{
	std::lock_guard<std::mutex> lock (mutex);
	return blocks.size ();
}
...
void xpeed::network::confirm_send (xpeed::confirm_ack const & confirm_a, std::shared_ptr<std::vector<uint8_t>> bytes_a, xpeed::endpoint const & endpoint_a)
{
	if (node.config.logging.network_publish_logging ())
	{
	...
	node.network.send_buffer (bytes_a->data (), bytes_a->size (), endpoint_a, [bytes_a, node_w, endpoint_a](boost::system::error_code const & ec, size_t size_a) {
		if (auto node_l = node_w.lock ())
		{
			...
			{
				node_l->stats.inc (xpeed::stat::type::message, xpeed::stat::detail::confirm_ack, xpeed::stat::dir::out);
			}
		}
	});
}

void xpeed::node::process_active (std::shared_ptr<xpeed::block> incoming)
{
	block_arrival.add (incoming->hash ());
	block_processor.add (incoming, xpeed::seconds_since_epoch ());
}

xpeed::process_return xpeed::node::process (xpeed::block const & block_a)
{
	auto transaction (store.tx_begin_write ());
...
void xpeed::node::keepalive_preconfigured (std::vector<std::string> const & peers_a)
{
	for (auto i (peers_a.begin ()), n (peers_a.end ()); i != n; ++i)
	{
		keepalive (*i, xpeed::network::node_port, true);
	}
}

xpeed::block_hash xpeed::node::latest (xpeed::account const & account_a)
{
	auto transaction (store.tx_begin_read ());
	return ledger.latest (transaction, account_a);
}

xpeed::uint128_t xpeed::node::balance (xpeed::account const & account_a)
{
	auto transaction (store.tx_begin_read ());
	return ledger.account_balance (transaction, account_a);
}

std::shared_ptr<xpeed::block> xpeed::node::block (xpeed::block_hash const & hash_a)
{
	auto transaction (store.tx_begin_read ());
	return store.block_get (transaction, hash_a);
}

std::pair<xpeed::uint128_t, xpeed::uint128_t> xpeed::node::balance_pending (xpeed::account const & account_a)
{
	std::pair<xpeed::uint128_t, xpeed::uint128_t> result;
	auto transaction (store.tx_begin_read ());
...
xpeed::uint128_t xpeed::node::weight (xpeed::account const & account_a)
{
	auto transaction (store.tx_begin_read ());
	return ledger.weight (transaction, account_a);
}

xpeed::account xpeed::node::representative (xpeed::account const & account_a)
{
	auto transaction (store.tx_begin_read ());
	xpeed::account_info info;
...
void xpeed::node::ongoing_keepalive ()
{
	...
	std::weak_ptr<xpeed::node> node_w (shared_from_this ());
...
void xpeed::node::ongoing_syn_cookie_cleanup ()
{
...
void xpeed::node::ongoing_rep_crawl ()
{
	auto now (std::chrono::steady_clock::now ());
...
void xpeed::node::ongoing_rep_calculation ()
{
	auto now (std::chrono::steady_clock::now ());
...
void xpeed::node::ongoing_bootstrap ()
{
...
void xpeed::node::ongoing_store_flush ()
{
	{
		auto transaction (store.tx_begin_write ());
...
void xpeed::node::ongoing_peer_store ()
{
	...
	if (!endpoint_peers.empty ())
	{
		...
		{
			xpeed::endpoint_key endpoint_key (endpoint.address ().to_v6 ().to_bytes (), endpoint.port ());
...
void xpeed::node::backup_wallet ()
{
	...
	for (auto i (wallets.items.begin ()), n (wallets.items.end ()); i != n; ++i)
	{
		...
		xpeed::set_secure_perm_directory (backup_path, error_chmod);
...
void xpeed::node::search_pending ()
{
...
void xpeed::node::bootstrap_wallet ()
{
	std::deque<xpeed::account> accounts;
	{
...
void xpeed::node::unchecked_cleanup ()
{
	std::deque<xpeed::unchecked_key> cleaning_list;
	// Collect old unchecked keys
	{
		...
		for (auto i (store.unchecked_begin (transaction)), n (store.unchecked_end ()); i != n && cleaning_list.size () < 128 * 1024 && xpeed::seconds_since_epoch () - now < 120; ++i)
		{
			xpeed::unchecked_key key (i->first);
			xpeed::unchecked_info info (i->second);
...
void xpeed::node::ongoing_unchecked_cleanup ()
{
...
int xpeed::node::price (xpeed::uint128_t const & balance_a, int amount_a)
{
	assert (balance_a >= amount_a * xpeed::Gxpd_ratio);
	auto balance_l (balance_a);
	double result (0.0);
	for (auto i (0); i < amount_a; ++i)
	{
		balance_l -= xpeed::Gxpd_ratio;
		auto balance_scaled ((balance_l / xpeed::Mxpd_ratio).convert_to<double> ());
...
{
public:
	distributed_work (std::shared_ptr<xpeed::node> const & node_a, xpeed::block_hash const & root_a, std::function<void(uint64_t)> callback_a, uint64_t difficulty_a) :
	distributed_work (1, node_a, root_a, callback_a, difficulty_a)
	{
		assert (node_a != nullptr);
	}
	distributed_work (unsigned int backoff_a, std::shared_ptr<xpeed::node> const & node_a, xpeed::block_hash const & root_a, std::function<void(uint64_t)> callback_a, uint64_t difficulty_a) :
	callback (callback_a),
	backoff (backoff_a),
	node (node_a),
	root (root_a),
		...
		if (!outstanding.empty ())
		{
			...
			for (auto const & i : outstanding)
			{
				...
				node->background ([this_l, host, service]() {
					...
					connection->socket.async_connect (xpeed::tcp_endpoint (host, service), [this_l, connection](boost::system::error_code const & ec) {
						if (!ec)
						{
	...
	void success (std::string const & body_a, boost::asio::ip::address const & address)
	{
		...
		try
		{
			...
			uint64_t work;
			if (!xpeed::from_string_hex (work_text, work))
			{
				if (!xpeed::work_validate (root, work))
				{
					set_once (work);
					stop ();
				}
		...
		if (last)
		{
			if (!completed.test_and_set ())
			{
				...
				else
				{
					...
					auto root_l (root);
					auto callback_l (callback);
					std::weak_ptr<xpeed::node> node_w (node);
...
void xpeed::node::work_generate_blocking (xpeed::block & block_a, uint64_t difficulty_a)
{
	block_a.block_work_set (work_generate_blocking (block_a.root (), difficulty_a));
}

void xpeed::node::work_generate (xpeed::uint256_union const & hash_a, std::function<void(uint64_t)> callback_a, uint64_t difficulty_a)
{
	auto work_generation (std::make_shared<distributed_work> (shared (), hash_a, callback_a, difficulty_a));
	work_generation->start ();
}

uint64_t xpeed::node::work_generate_blocking (xpeed::uint256_union const & hash_a, uint64_t difficulty_a)
{
...
void xpeed::node::add_initial_peers ()
{
	auto transaction (store.tx_begin_read ());
	for (auto i (store.peers_begin (transaction)), n (store.peers_end ()); i != n; ++i)
	{
		xpeed::endpoint endpoint (boost::asio::ip::address_v6 (i->first.address_bytes ()), i->first.port ());
...
void xpeed::node::block_confirm (std::shared_ptr<xpeed::block> block_a)
{
...
xpeed::uint128_t xpeed::node::delta ()
{
	auto result ((online_reps.online_stake () / 100) * config.online_weight_quorum);
	return result;
}

void xpeed::node::ongoing_online_weight_calculation_queue ()
{
	std::weak_ptr<xpeed::node> node_w (shared_from_this ());
	alarm.add (std::chrono::steady_clock::now () + (std::chrono::seconds (xpeed::online_reps::weight_period)), [node_w]() {
		if (auto node_l = node_w.lock ())
		{
...
void xpeed::node::ongoing_online_weight_calculation ()
{
	online_reps.sample ();
	ongoing_online_weight_calculation_queue ();
}
...
namespace
{
class confirmed_visitor : public xpeed::block_visitor
{
public:
	confirmed_visitor (xpeed::transaction const & transaction_a, xpeed::node & node_a, std::shared_ptr<xpeed::block> block_a, xpeed::block_hash const & hash_a) :
	...
	void scan_receivable (xpeed::account const & account_a)
	{
		for (auto i (node.wallets.items.begin ()), n (node.wallets.items.end ()); i != n; ++i)
		{
			...
			if (wallet->store.exists (transaction_l, account_a))
			{
				...
				auto error (node.store.pending_get (transaction, xpeed::pending_key (account_a, hash), pending));
				if (!error)
				{
					auto node_l (node.shared ());
					auto amount (pending.amount.number ());
					wallet->receive_async (block, representative, amount, [](std::shared_ptr<xpeed::block>) {});
				}
				else
				{
	...
	xpeed::block_hash const & hash;
};
}

void xpeed::node::process_confirmed (std::shared_ptr<xpeed::block> block_a, uint8_t iteration)
{
	auto hash (block_a->hash ());
	if (ledger.block_exists (block_a->type (), hash))
	{
		...
		xpeed::account pending_account (0);
		if (auto state = dynamic_cast<xpeed::state_block *> (block_a.get ()))
		{
			is_state_send = ledger.is_send (transaction, *state);
			pending_account = state->hashables.link;
		}
		if (auto send = dynamic_cast<xpeed::send_block *> (block_a.get ()))
		{
	...
	{
		iteration++;
		std::weak_ptr<xpeed::node> node_w (shared ());
...
void xpeed::node::process_message (xpeed::message & message_a, xpeed::endpoint const & sender_a)
{
	network_message_visitor visitor (*this, sender_a);
	message_a.visit (visitor);
}

xpeed::endpoint xpeed::network::endpoint ()
{
	boost::system::error_code ec;
	std::unique_lock<std::mutex> lock (socket_mutex);
	...
	return xpeed::endpoint (boost::asio::ip::address_v6::loopback (), port);
}

bool xpeed::block_arrival::add (xpeed::block_hash const & hash_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	auto now (std::chrono::steady_clock::now ());
	auto inserted (arrival.insert (xpeed::block_arrival_info{ now, hash_a }));
	auto result (!inserted.second);
	return result;
}

bool xpeed::block_arrival::recent (xpeed::block_hash const & hash_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	auto now (std::chrono::steady_clock::now ());
...
xpeed::online_reps::online_reps (xpeed::ledger & ledger_a, xpeed::uint128_t minimum_a) :
ledger (ledger_a),
minimum (minimum_a)
{
...
void xpeed::online_reps::observe (xpeed::account const & rep_a)
{
	auto transaction (ledger.store.tx_begin_read ());
	if (ledger.weight (transaction, rep_a) > xpeed::Gxpd_ratio)
	{
		std::lock_guard<std::mutex> lock (mutex);
		reps.insert (rep_a);
	}
...
void xpeed::online_reps::sample ()
{
	...
	xpeed::uint128_t current;
	std::unordered_set<xpeed::account> reps_copy;
	{
		std::lock_guard<std::mutex> lock (mutex);
		reps_copy.swap (reps);
	}
...
xpeed::uint128_t xpeed::online_reps::trend (xpeed::transaction & transaction_a)
{
	std::vector<xpeed::uint128_t> items;
	...
	return xpeed::uint128_t{ items[median_idx] };
}

xpeed::uint128_t xpeed::online_reps::online_stake ()
{
	std::lock_guard<std::mutex> lock (mutex);
	return std::max (online, minimum);
}

std::vector<xpeed::account> xpeed::online_reps::list ()
{
...
bool xpeed::reserved_address (xpeed::endpoint const & endpoint_a, bool blacklist_loopback)
{
	assert (endpoint_a.address ().is_v6 ());
...
void xpeed::network::send_buffer (uint8_t const * data_a, size_t size_a, xpeed::endpoint const & endpoint_a, std::function<void(boost::system::error_code const &, size_t)> callback_a)
{
	std::unique_lock<std::mutex> lock (socket_mutex);
	if (node.config.logging.network_packet_logging ())
	{
	...
	if (on.load ())
	{
		socket.async_send_to (boost::asio::buffer (data_a, size_a), endpoint_a, [this, callback_a](boost::system::error_code const & ec, size_t size_a) {
			...
			this->node.stats.add (xpeed::stat::type::traffic, xpeed::stat::dir::out, size_a);
			if (ec == boost::system::errc::host_unreachable)
			{
				this->node.stats.inc (xpeed::stat::type::error, xpeed::stat::detail::unreachable_host, xpeed::stat::dir::out);
			}
			if (this->node.config.logging.network_packet_logging ())
			{
...
std::shared_ptr<xpeed::node> xpeed::node::shared ()
{
	return shared_from_this ();
}

xpeed::election_vote_result::election_vote_result () :
replay (false),
processed (false)
{
}

xpeed::election_vote_result::election_vote_result (bool replay_a, bool processed_a)
{
	replay = replay_a;
	processed = processed_a;
}

xpeed::election::election (xpeed::node & node_a, std::shared_ptr<xpeed::block> block_a, std::function<void(std::shared_ptr<xpeed::block>)> const & confirmation_action_a) :
...
announcements (0)
{
	last_votes.insert (std::make_pair (xpeed::not_an_account (), xpeed::vote_info{ std::chrono::steady_clock::now (), 0, block_a->hash () }));
	blocks.insert (std::make_pair (block_a->hash (), block_a));
}

void xpeed::election::compute_rep_votes (xpeed::transaction const & transaction_a)
{
	if (node.config.enable_voting)
	{
		node.wallets.foreach_representative (transaction_a, [this, &transaction_a](xpeed::public_key const & pub_a, xpeed::raw_key const & prv_a) {
...
void xpeed::election::confirm_once (xpeed::transaction const & transaction_a, bool confirmed_back)
{
...
void xpeed::election::confirm_back (xpeed::transaction const & transaction_a)
{
	std::deque<xpeed::block_hash> hashes = { status.winner->previous (), status.winner->source (), status.winner->link () };
	while (!hashes.empty ())
	{
...
void xpeed::election::stop ()
{
	stopped = true;
}

bool xpeed::election::have_quorum (xpeed::tally_t const & tally_a, xpeed::uint128_t tally_sum)
{
	bool result = false;
...
xpeed::tally_t xpeed::election::tally (xpeed::transaction const & transaction_a)
{
	std::unordered_map<xpeed::block_hash, xpeed::uint128_t> block_weights;
...
void xpeed::election::confirm_if_quorum (xpeed::transaction const & transaction_a)
{
...
void xpeed::election::log_votes (xpeed::tally_t const & tally_a)
{
...
xpeed::election_vote_result xpeed::election::vote (xpeed::account rep, uint64_t sequence, xpeed::block_hash block_hash)
{
	...
	if (xpeed::is_test_network || weight > supply / 1000) // 0.1% or above
	{
	...
	return xpeed::election_vote_result (replay, should_process);
}

bool xpeed::node::validate_block_by_previous (xpeed::transaction const & transaction, std::shared_ptr<xpeed::block> block_a)
{
	bool result (false);
	xpeed::account account;
	...
	if (!result && block_a->type () == xpeed::block_type::state)
	{
		std::shared_ptr<xpeed::state_block> block_l (std::static_pointer_cast<xpeed::state_block> (block_a));
	...
	if (!result && (account.is_zero () || xpeed::validate_message (account, block_a->hash (), block_a->block_signature ())))
	{
		result = true;
	}
	return result;
...
bool xpeed::election::publish (std::shared_ptr<xpeed::block> block_a)
{
	auto result (false);
	if (blocks.size () >= 10)
	{
...
size_t xpeed::election::last_votes_size ()
{
	std::lock_guard<std::mutex> lock (node.active.mutex);
	return last_votes.size ();
}

void xpeed::active_transactions::request_confirm (std::unique_lock<std::mutex> & lock_a)
{
	std::unordered_set<xpeed::uint512_union> inactive;
	auto transaction (node.store.tx_begin_read ());
	unsigned unconfirmed_count (0);
	unsigned unconfirmed_announcements (0);
	std::unordered_map<xpeed::endpoint, std::vector<std::pair<xpeed::block_hash, xpeed::block_hash>>> requests_bundle;
	std::deque<std::shared_ptr<xpeed::block>> rebroadcast_bundle;
	std::deque<std::pair<std::shared_ptr<xpeed::block>, std::shared_ptr<std::vector<xpeed::peer_information>>>> confirm_req_bundle;
	...
	for (auto i (roots.get<1> ().begin ()), n (roots.get<1> ().end ()); i != n; ++i)
	{
		...
		else
		{
			if (election_l->announcements > announcement_long)
			{
				...
				if (election_l->announcements % announcement_long == 1 && roots_size < 100 && !xpeed::is_test_network)
				{
					std::shared_ptr<xpeed::block> previous;
			...
			{
				auto reps (std::make_shared<std::vector<xpeed::peer_information>> (node.peers.representatives (std::numeric_limits<size_t>::max ())));
				std::unordered_set<xpeed::account> probable_reps;
				xpeed::uint128_t total_weight (0);
					...
					else
					{
						for (auto & rep : *reps)
						{
							...
							if (rep_request == requests_bundle.end ())
							{
								...
								{
									std::vector<std::pair<xpeed::block_hash, xpeed::block_hash>> insert_vector = { root_hash };
									requests_bundle.insert (std::make_pair (rep.endpoint, insert_vector));
								}
							}
							else if (rep_request->second.size () < max_broadcast_queue * xpeed::network::confirm_req_hashes_max)
							{
				...
				else
				{
					if (!xpeed::is_test_network)
					{
						confirm_req_bundle.push_back (std::make_pair (election_l->status.winner, std::make_shared<std::vector<xpeed::peer_information>> (node.peers.list_vector (100))));
					}
					else
					{
						for (auto & rep : *reps)
	...
	if (!xpeed::is_live_network && !requests_bundle.empty ())
	{
...
void xpeed::active_transactions::request_loop ()
{
	std::unique_lock<std::mutex> lock (mutex);
	started = true;
...
void xpeed::active_transactions::stop ()
{
	std::unique_lock<std::mutex> lock (mutex);
	while (!started)
	{
...
bool xpeed::active_transactions::start (std::shared_ptr<xpeed::block> block_a, std::function<void(std::shared_ptr<xpeed::block>)> const & confirmation_action_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	return add (block_a, confirmation_action_a);
}

bool xpeed::active_transactions::add (std::shared_ptr<xpeed::block> block_a, std::function<void(std::shared_ptr<xpeed::block>)> const & confirmation_action_a)
{
	auto error (true);
	if (!stopped)
	{
		auto root (xpeed::uint512_union (block_a->previous (), block_a->root ()));
		auto existing (roots.find (root));
		if (existing == roots.end ())
		{
			auto election (std::make_shared<xpeed::election> (node, block_a, confirmation_action_a));
			uint64_t difficulty (0);
			auto error (xpeed::work_validate (*block_a, &difficulty));
			release_assert (!error);
			roots.insert (xpeed::conflict_info{ root, difficulty, election });
...
bool xpeed::active_transactions::vote (std::shared_ptr<xpeed::vote> vote_a, bool single_lock)
{
	...
	bool processed (false);
	{
		...
		{
			xpeed::election_vote_result result;
			if (vote_block.which ())
			{
				auto block_hash (boost::get<xpeed::block_hash> (vote_block));
			...
			{
				auto block (boost::get<std::shared_ptr<xpeed::block>> (vote_block));
				auto existing (roots.find (xpeed::uint512_union (block->previous (), block->root ())));
				if (existing != roots.end ())
				{
...
bool xpeed::active_transactions::active (xpeed::block const & block_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	return roots.find (xpeed::uint512_union (block_a.previous (), block_a.root ())) != roots.end ();
}

void xpeed::active_transactions::update_difficulty (xpeed::block const & block_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	auto existing (roots.find (xpeed::uint512_union (block_a.previous (), block_a.root ())));
	if (existing != roots.end ())
	{
		uint64_t difficulty;
		auto error (xpeed::work_validate (block_a, &difficulty));
		assert (!error);
		roots.modify (existing, [difficulty](xpeed::conflict_info & info_a) {
...
std::deque<std::shared_ptr<xpeed::block>> xpeed::active_transactions::list_blocks (bool single_lock)
{
	std::deque<std::shared_ptr<xpeed::block>> result;
	std::unique_lock<std::mutex> lock;
	if (!single_lock)
	{
		lock = std::unique_lock<std::mutex> (mutex);
...
std::deque<xpeed::election_status> xpeed::active_transactions::list_confirmed ()
{
	std::lock_guard<std::mutex> lock (mutex);
	return confirmed;
}

void xpeed::active_transactions::erase (xpeed::block const & block_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	if (roots.find (xpeed::uint512_union (block_a.previous (), block_a.root ())) != roots.end ())
	{
		roots.erase (xpeed::uint512_union (block_a.previous (), block_a.root ()));
...
bool xpeed::active_transactions::empty ()
{
	std::lock_guard<std::mutex> lock (mutex);
	return roots.empty ();
}

size_t xpeed::active_transactions::size ()
{
	std::lock_guard<std::mutex> lock (mutex);
	return roots.size ();
}

xpeed::active_transactions::active_transactions (xpeed::node & node_a) :
node (node_a),
started (false),
stopped (false),
thread ([this]() {
	xpeed::thread_role::set (xpeed::thread_role::name::request_loop);
	request_loop ();
})
{
	std::unique_lock<std::mutex> lock (mutex);
...
xpeed::active_transactions::~active_transactions ()
{
	stop ();
}

bool xpeed::active_transactions::publish (std::shared_ptr<xpeed::block> block_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	auto existing (roots.find (xpeed::uint512_union (block_a->previous (), block_a->root ())));
	auto result (true);
...
int xpeed::node::store_version ()
{
	auto transaction (store.tx_begin_read ());
	return store.version_get (transaction);
}

xpeed::thread_runner::thread_runner (boost::asio::io_context & io_ctx_a, unsigned service_threads_a)
{
	boost::thread::attributes attrs;
	xpeed::thread_attributes::set (attrs);
	for (auto i (0u); i < service_threads_a; ++i)
	{
		threads.push_back (boost::thread (attrs, [&io_ctx_a]() {
			xpeed::thread_role::set (xpeed::thread_role::name::io);
			try
			{
				io_ctx_a.run ();
			}
...
xpeed::thread_runner::~thread_runner ()
{
	join ();
}

void xpeed::thread_runner::join ()
{
	for (auto & i : threads)
	{
		if (i.joinable ())
...
xpeed::inactive_node::inactive_node (boost::filesystem::path const & path, uint16_t peering_port_a) :
path (path),
...
peering_port (peering_port_a)
{
	...
	xpeed::set_secure_perm_directory (path, error_chmod);
	logging.max_size = std::numeric_limits<std::uintmax_t>::max ();
	logging.init (path);
	node = std::make_shared<xpeed::node> (init, *io_context, peering_port, path, alarm, logging, work);
}

xpeed::inactive_node::~inactive_node ()
{
	node->stop ();
}

xpeed::udp_buffer::udp_buffer (xpeed::stat & stats, size_t size, size_t count) :
stats (stats),
free (count),
full (count),
slab (size * count),
...
stopped (false)
{
	...
	{
		*entry_data = { slab_data + i * size, 0, xpeed::endpoint () };
		free.push_back (entry_data);
	}
}
xpeed::udp_data * xpeed::udp_buffer::allocate ()
{
	std::unique_lock<std::mutex> lock (mutex);
	while (!stopped && free.empty () && full.empty ())
	{
		stats.inc (xpeed::stat::type::udp, xpeed::stat::detail::blocking, xpeed::stat::dir::in);
		condition.wait (lock);
	}
	xpeed::udp_data * result (nullptr);
	if (!free.empty ())
	{
		result = free.front ();
		free.pop_front ();
	...
	if (result == nullptr)
	{
		result = full.front ();
		full.pop_front ();
		stats.inc (xpeed::stat::type::udp, xpeed::stat::detail::overflow, xpeed::stat::dir::in);
	}
	return result;
}
void xpeed::udp_buffer::enqueue (xpeed::udp_data * data_a)
{
...
xpeed::udp_data * xpeed::udp_buffer::dequeue ()
{
	std::unique_lock<std::mutex> lock (mutex);
...
void xpeed::udp_buffer::release (xpeed::udp_data * data_a)
{
	assert (data_a != nullptr);
	{
		std::lock_guard<std::mutex> lock (mutex);

...
class jsonconfig : public xpeed::error_aware<>
{
public:
	jsonconfig () :
	tree (tree_default)
	{
		error = std::make_shared<xpeed::error> ();
	}

	jsonconfig (boost::property_tree::ptree & tree_a, std::shared_ptr<xpeed::error> error_a = nullptr) :
	 ...
	 * @return xpeed::error&, including a descriptive error message if the config file is malformed.
	 */
	template <typename T>
	xpeed::error & read_and_update (T & object, boost::filesystem::path const & path_a)
	{
		std::fstream stream;
	...
	void open_or_create (std::fstream & stream_a, std::string const & path_a)
	{
		if (!boost::filesystem::exists (path_a))
		{
			...
			xpeed::set_secure_perm_file (path_a);
		}
	...
	jsonconfig get_required_child (std::string const & key_a)
	{
		...
		if (!child)
		{
			*error = xpeed::error_config::missing_value;
			error->set_message ("Missing configuration node: " + key_a);
		}
		return child ? jsonconfig (child.get (), error) : *this;
	}

	jsonconfig & put_child (std::string const & key_a, xpeed::jsonconfig & conf_a)
	{
		tree.add_child (key_a, conf_a.get_tree ());
		return *this;
	}

	jsonconfig & replace_child (std::string const & key_a, xpeed::jsonconfig & conf_a)
	{
		tree.erase (key_a);
	 ...
	 * @return May return xpeed::error_config::invalid_value
	 */
	template <typename T>
	 ...
	 * @note May set xpeed::error_config::missing_value if \p key is missing, xpeed::error_config::invalid_value if value is invalid.
	 */
	template <typename T>
	...
	xpeed::error & get_error () override
	{
		return *error;
	}
...
protected:
	...
	template <typename T, typename = std::enable_if_t<xpeed::is_lexical_castable<T>::value>>
	jsonconfig & get_config (bool optional, std::string key, T & target, T default_value = T ())
	{
		try
		{
			...
			{
				conditionally_set_error<T> (xpeed::error_config::invalid_value, optional, key);
			}
		}
		catch (boost::property_tree::ptree_bad_path const &)
		{
			if (!optional)
			{
				conditionally_set_error<T> (xpeed::error_config::missing_value, optional, key);
			}
			else
			{
...
private:
	...
	/** We're a xpeed::error_aware type. Child nodes share the error state. */
	std::shared_ptr<xpeed::error> error;
};
}

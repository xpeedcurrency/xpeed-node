...
public:
	...
	timer (xpeed::timer_state state_a, std::string description_a = "timer") :
	desc (description_a)
	{
		if (state_a == xpeed::timer_state::started)
		{
			start ();
		}
	}
	...
	void start ()
	{
		assert (state == xpeed::timer_state::stopped);
		state = xpeed::timer_state::started;
		begin = CLOCK::now ();
	}
	...
	UNIT stop ()
	{
		assert (state == xpeed::timer_state::started);
		state = xpeed::timer_state::stopped;
	...
	xpeed::timer_state current_state () const
	{
		return state;
	}
...
private:
	...
	xpeed::timer_state state{ xpeed::timer_state::stopped };
	std::string desc;
...
class autotimer : public xpeed::timer<UNIT>
{
public:
	autotimer (std::string description_a, std::ostream & stream_a = std::cout) :
	xpeed::timer<UNIT> (description_a), stream (stream_a)
	{
		xpeed::timer<UNIT>::start ();
	}
	~autotimer ()
	{
		xpeed::timer<UNIT>::stop (stream);
	}

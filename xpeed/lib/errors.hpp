...
#include <cassert>
#include <memory>
#include <xpeed/lib/expected.hpp>
#include <string>
...
REGISTER_ERROR_CODES (xpeed, error_common);
REGISTER_ERROR_CODES (xpeed, error_blocks);
REGISTER_ERROR_CODES (xpeed, error_rpc);
REGISTER_ERROR_CODES (xpeed, error_process);
REGISTER_ERROR_CODES (xpeed, error_config);
...
inline std::error_code make_error_code (boost::system::errc::errc_t e)
{
	...
	::xpeed::error_conversion::generic_category ());
}
}
namespace xpeed
{
namespace error_conversion
{
	...
	inline std::error_code convert (const boost::system::error_code & error)
	{
		...
		return xpeed::error_common::invalid_type_conversion;
	}
}
}
...
public:
	error () = default;
	error (xpeed::error const & error_a) = default;
	error (xpeed::error && error_a) = default;
	...
	{
		code = xpeed::error_common::generic;
		message = std::move (message_a);
	}

	error (std::exception const & exception_a)
	{
		code = xpeed::error_common::exception;
		message = exception_a.what ();
	}

	error & operator= (xpeed::error const & err_a)
	{
		code = err_a.code;
	...
	error & operator= (xpeed::error && err_a)
	{
		code = err_a.code;
	...
	{
		code = xpeed::error_conversion::convert (code_a);
		message.clear ();
		return *this;
	}
	...
	{
		code = xpeed::error_conversion::convert (boost::system::errc::make_error_code (code_a));
		message.clear ();
		return *this;
	}

	/** Set the error to xpeed::error_common::generic and the error message to \p message_a */
	...
	/** Set the error to xpeed::error_common::generic. */
	...
	/** Sets the error to xpeed::error_common::exception and adopts the exception error message. */
	...
	error & then (std::function<xpeed::error &()> next)
	{
	...
	error & set (std::string message_a, std::error_code code_a = xpeed::error_common::generic)
	{
		message = message_a;
		code = code_a;
		return *this;
	...
	/** Set a custom error message. If the error code is not set, it will be set to xpeed::error_common::generic. */
...
/**
 * A type that manages a xpeed::error.
 * The default return type is xpeed::error&, though shared_ptr<xpeed::error> is a good option in cases
 * where shared error state is desirable.
 */
template <typename RET_TYPE = xpeed::error &>
class error_aware
{
	static_assert (std::is_same<RET_TYPE, xpeed::error &>::value || std::is_same<RET_TYPE, std::shared_ptr<xpeed::error>>::value, "Must be xpeed::error& or shared_ptr<xpeed::error>");

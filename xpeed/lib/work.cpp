...
bool xpeed::work_validate (xpeed::block_hash const & root_a, uint64_t work_a, uint64_t * difficulty_a)
{
	auto value (xpeed::work_value (root_a, work_a));
	if (difficulty_a != nullptr)
	{
		*difficulty_a = value;
	}
	return value < xpeed::work_pool::publish_threshold;
}

bool xpeed::work_validate (xpeed::block const & block_a, uint64_t * difficulty_a)
{
	return work_validate (block_a.root (), block_a.block_work (), difficulty_a);
}

uint64_t xpeed::work_value (xpeed::block_hash const & root_a, uint64_t work_a)
{
	uint64_t result;
	blake2b_state hash;
...
xpeed::work_pool::work_pool (unsigned max_threads_a, std::function<boost::optional<uint64_t> (xpeed::uint256_union const &)> opencl_a) :
ticket (0),
done (false),
opencl (opencl_a)
{
	...
	auto count (xpeed::is_test_network ? 1 : std::min (max_threads_a, std::max (1u, boost::thread::hardware_concurrency ())));
	for (auto i (0); i < count; ++i)
	{
		auto thread (boost::thread (attrs, [this, i]() {
			xpeed::thread_role::set (xpeed::thread_role::name::work);
			xpeed::work_thread_reprioritize ();
			loop (i);
		}));
...
void xpeed::work_pool::loop (uint64_t thread)
{
	// Quick RNG for work attempts.
	xorshift1024star rng;
	xpeed::random_pool::generate_block (reinterpret_cast<uint8_t *> (rng.s.data ()), rng.s.size () * sizeof (decltype (rng.s)::value_type));
	uint64_t work;
	uint64_t output;
	blake2b_state hash;
	...
	while (!done || !pending.empty ())
	{
		...
		if (!empty)
		{
			...
			if (ticket == ticket_l)
			{
				...
				assert (output >= xpeed::work_pool::publish_threshold);
...
void xpeed::work_pool::cancel (xpeed::uint256_union const & root_a)
{
...
void xpeed::work_pool::generate (xpeed::uint256_union const & root_a, std::function<void(boost::optional<uint64_t> const &)> callback_a, uint64_t difficulty_a)
{
...
uint64_t xpeed::work_pool::generate (xpeed::uint256_union const & hash_a, uint64_t difficulty_a)
{

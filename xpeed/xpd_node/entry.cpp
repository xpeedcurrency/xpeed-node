...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("debug_profile_process", "Profile active blocks processing (only for xpd_test_network)")
		("debug_profile_votes", "Profile votes processing (only for xpd_test_network)")
	...
	boost::filesystem::path data_path ((data_path_it != vm.end ()) ? data_path_it->second.as<std::string> () : xpeed::working_path ());
	auto ec = xpeed::handle_node_options (vm);
	if (ec == xpeed::error_cli::unknown_command)
	{
		if (vm.count ("daemon") > 0)
		{
			xpd_daemon::daemon daemon;
		...
		{
			xpeed::inactive_node node (data_path);
		...
		else if (vm.count ("debug_bootstrap_generate"))
		{
			...
			if (key_it != vm.end ())
			{
				...
				{
					xpeed::keypair genesis (key.to_string ());
					xpeed::work_pool work (std::numeric_limits<unsigned>::max (), nullptr);
					std::cout << "Genesis: " << genesis.prv.data.to_string () << "\n"
					          << "Public: " << genesis.pub.to_string () << "\n"
					          << "Account: " << genesis.pub.to_account () << "\n";
					xpeed::keypair landing;
					...
					xpeed::open_block genesis_block (genesis.pub, genesis.pub, genesis.pub, genesis.prv, genesis.pub, work.generate (genesis.pub));
					std::cout << genesis_block.to_json ();
					std::cout.flush ();
					xpeed::block_hash previous (genesis_block.hash ());
					for (auto i (0); i != 8; ++i)
					{
						xpeed::uint128_t yearly_distribution (xpeed::uint128_t (1) << (127 - (i == 7 ? 6 : i)));
						...
						for (auto j (0); j != 52; ++j)
						{
							...
							xpeed::send_block send (previous, landing.pub, balance, genesis.prv, genesis.pub, work.generate (previous));
							previous = send.hash ();
		...
		else if (vm.count ("debug_dump_representatives"))
		{
			...
			std::map<xpeed::account, xpeed::uint128_t> calculated;
			for (auto i (node.node->store.latest_begin (transaction)), n (node.node->store.latest_end ()); i != n; ++i)
			{
				xpeed::account_info info (i->second);
				xpeed::block_hash rep_block (node.node->ledger.representative_calculated (transaction, info.head));
		...
		else if (vm.count ("debug_profile_kdf"))
		{
			...
			xpeed::uint256_union salt (0);
			std::string password ("");
			while (true)
			{
				auto begin1 (std::chrono::high_resolution_clock::now ());
				auto success (argon2_hash (1, xpeed::wallet_store::kdf_work, 1, password.data (), password.size (), salt.bytes.data (), salt.bytes.size (), result.bytes.data (), result.bytes.size (), NULL, 0, Argon2_d, 0x10));
				(void)success;
		...
		{
			xpeed::work_pool work (std::numeric_limits<unsigned>::max (), nullptr);
			xpeed::change_block block (0, 0, xpeed::keypair ().prv, 0, 0);
				...
				if (!result)
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							...
							auto opencl (xpeed::opencl_work::create (true, { platform, device, threads }, logging));
							xpeed::work_pool work_pool (std::numeric_limits<unsigned>::max (), opencl ? [&opencl](xpeed::uint256_union const & root_a) {
		...
		{
			xpeed::keypair key;
			xpeed::uint256_union message;
			xpeed::uint512_union signature;
			signature = xpeed::sign_message (key.prv, key.pub, message);
			auto begin (std::chrono::high_resolution_clock::now ());
			for (auto i (0u); i < 1000; ++i)
			{
				xpeed::validate_message (key.pub, message, signature);
			}
		...
		else if (vm.count ("debug_verify_profile_batch"))
		{
			...
			xpeed::uint256_union message;
			xpeed::uint512_union signature (xpeed::sign_message (key.prv, key.pub, message));
			...
			auto begin (std::chrono::high_resolution_clock::now ());
			xpeed::validate_message_batch (messages.data (), lengths.data (), pub_keys.data (), signatures.data (), batch_count, verifications.data ());
			auto end (std::chrono::high_resolution_clock::now ());
		...
		else if (vm.count ("debug_profile_sign"))
		{
			...
			{
				xpeed::keypair key;
				xpeed::block_hash latest (0);
				auto begin1 (std::chrono::high_resolution_clock::now ());
				for (uint64_t balance (0); balance < 1000; ++balance)
				{
					xpeed::send_block send (latest, key.pub, balance, key.prv, key.pub, 0);
					latest = send.hash ();
				}
				auto end1 (std::chrono::high_resolution_clock::now ());
				std::cerr << boost::str (boost::format ("%|1$ 12d|\n") % std::chrono::duration_cast<std::chrono::microseconds> (end1 - begin1).count ());
		...
		else if (vm.count ("debug_profile_process"))
		{
			if (xpeed::is_test_network)
			{
				...
				xpeed::work_pool work (std::numeric_limits<unsigned>::max (), nullptr);
				xpeed::logging logging;
				auto path (xpeed::unique_path ());
				logging.init (path);
				auto node (std::make_shared<xpeed::node> (init, system.io_ctx, 24001, path, system.alarm, logging, work));
				xpeed::block_hash genesis_latest (node->latest (xpeed::test_genesis_key.pub));
				xpeed::uint128_t genesis_balance (std::numeric_limits<xpeed::uint128_t>::max ());
				// Generating keys
				std::vector<xpeed::keypair> keys (num_accounts);
				std::vector<xpeed::block_hash> frontiers (num_accounts);
				std::vector<xpeed::uint128_t> balances (num_accounts, 1000000000);
				// Generating blocks
				std::deque<std::shared_ptr<xpeed::block>> blocks;
				for (auto i (0); i != num_accounts; ++i)
				{
					genesis_balance = genesis_balance - 1000000000;

					auto send = builder.state ()
					            .account (xpeed::test_genesis_key.pub)
					            .previous (genesis_latest)
					            .representative (xpeed::test_genesis_key.pub)
					...
					auto open = builder.state ()
					            ...
					            .balance (balances[i])
					            .link (genesis_latest)
					            .sign (xpeed::test_genesis_key.prv, xpeed::test_genesis_key.pub)
			...
			{
				std::cerr << "For this test ACTIVE_NETWORK should be xpeed_test_network" << std::endl;
			}
		}
		else if (vm.count ("debug_profile_votes"))
		{
			if (xpeed::is_test_network)
			{
				...
				xpeed::uint128_t genesis_balance (std::numeric_limits<xpeed::uint128_t>::max ());
				// Generating keys
				std::vector<xpeed::keypair> keys (num_representatives);
				xpeed::uint128_t balance ((node->config.online_weight_minimum.number () / num_representatives) + 1);
				...
				// Generating votes
				std::deque<std::shared_ptr<xpeed::vote>> votes;
				for (auto j (0); j != num_representatives; ++j)
				{
					uint64_t sequence (1);
					for (auto & i : blocks)
					{
						auto vote (std::make_shared<xpeed::vote> (keys[j].pub, keys[j].prv, sequence, std::vector<xpeed::block_hash> (1, i->hash ())));
			...
			{
				std::cerr << "For this test ACTIVE_NETWORK should be xpd_test_network" << std::endl;
			}
		}
		else if (vm.count ("debug_rpc"))
		{
			...
			xpeed::inactive_node inactive_node_l (data_path);
			xpeed::rpc_config rpc_config_l;
			rpc_config_l.enable_control = true;
			std::unique_ptr<xpeed::rpc> rpc_l = get_rpc (inactive_node_l.node->io_ctx, *inactive_node_l.node, rpc_config_l);
			std::string req_id_l ("1");
			xpeed::rpc_handler handler_l (*inactive_node_l.node, *rpc_l, command_l.str (), req_id_l, response_handler_l);
		...
		else if (vm.count ("debug_validate_blocks"))
		{
			...
			for (auto i (node.node->store.latest_begin (transaction)), n (node.node->store.latest_end ()); i != n; ++i)
			{
				...
				xpeed::block_hash calculated_hash (0);
				...
				while (!hash.is_zero ())
				{
					...
					if (block->type () == xpeed::block_type::open || block->type () == xpeed::block_type::state)
					{
					...
					if (validate_message (account, hash, block->block_signature ()))
					{
						bool invalid (true);
						// Epoch blocks
						if (!node.node->ledger.epoch_link.is_zero () && block->type () == xpeed::block_type::state)
						{
							auto & state_block (static_cast<xpeed::state_block &> (*block.get ()));
					...
					if (xpeed::work_validate (*block.get ()))
					{
						std::cerr << boost::str (boost::format ("Invalid work for block %1% value: %2%\n") % hash.to_string () % xpeed::to_string_hex (block->block_work ()));
					}
			...
			for (auto i (node.node->store.pending_begin (transaction)), n (node.node->store.pending_end ()); i != n; ++i)
			{
				...
				else
				{
					...
					xpeed::account destination (0);
					if (auto state = dynamic_cast<xpeed::state_block *> (block.get ()))
					{
					...
					else if (auto send = dynamic_cast<xpeed::send_block *> (block.get ()))
					{
		...
		{
			xpeed::inactive_node node2 (xpeed::unique_path (), 24001);
			...
			size_t count (0);
			{
				xpeed::inactive_node node (data_path, 24000);
				auto transaction (node.node->store.tx_begin ());
				...
				for (auto i (node.node->store.latest_begin (transaction)), n (node.node->store.latest_end ()); i != n; ++i)
				{
					...
					while (!hash.is_zero ())
					{
						...
						if (block != nullptr)
						{
							...
							xpeed::unchecked_info unchecked_info (block, account, 0, xpeed::signature_verification::unknown);
		...
		else if (vm.count ("debug_peers"))
		{
			...
			{
				std::cout << boost::str (boost::format ("%1%\n") % xpeed::endpoint (boost::asio::ip::address_v6 (i->first.address_bytes ()), i->first.port ()));
			}
		}
		else if (vm.count ("version"))
		{
			...
			{
				std::cout << "Version " << XPD_MAJOR_MINOR_VERSION << std::endl;
			}
			else
			{
				std::cout << "Version " << XPD_MAJOR_MINOR_RC_VERSION << std::endl;
			}
		}
		else
		{
